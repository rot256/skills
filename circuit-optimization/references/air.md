...
Another word-equality gadget is the same idea with squares: `diff = sum (a_i - b_i)^2`, then one inverse and one flag --
...
on a `State` interaction -- `receive_state(clk, pc)`, `send_state(clk + delta, next_pc)`.
...
and *doubles* the composition trace even if the other 51 constraints are degree 3 (ethSTARK section 3.6.1).
...
and lets the same flag be both the space tag and the read multiplicity: `rs2_as in {0,1}` is passed **directly** as the
...
The CRT/bound discipline is shared with R1CS (`r1cs.md` Part IV and `techniques.md` section 2). What is AIR-specific is that the
