...
A degree-$d$ constraint composed with degree-$(n-1)$ trace polynomials has degree $d(n-1)$ -- or $d(n-1)+1$ once the linear transition selector is folded in.
Dividing by the vanishing polynomial $Z_H$ leaves $\approx (d-1)n$, which does not fit in one column of height $n$ and is therefore committed in **chunks**, whose count is **rounded up to a power of two** because the chunk domains must be cosets of a subgroup:
...
**Price.** $2^{\lceil\log_2(D-1)\rceil}$ chunks, each $E$ base-field columns of height $n$, all LDE'd, committed and opened.
...
Crossing it costs one `coset_idft_batch` over the full $W$-wide LDE plus one `coset_dft_batch` -- roughly a doubling of the prover's DFT bill for a 300-column AIR.

**Deployed configurations sit exactly on the cliff, at $D - 1 = \text{blowup}$.** One shipping config pairs `rate_bits: 3` with `max_quotient_degree_factor: 8`, tuned so that `max_fft_points = 1 << (degree_bits + max(rate_bits, log2_ceil(quotient_degree_factor)))` is a *tie*; another pairs inverse rate 4 with 4 quotient chunks.
...
**Worked.** A PLONKish config with 80 routed wires at $D=8$: $\lceil 80/8\rceil - 1 = 9$ partial-product columns, $\times\,2$ challenges = **18 columns**.
At $D=2$: $\lceil 80/2\rceil - 1 = 39$, $\times 2$ = **78 columns**.
...
```
Each inline deletes one column $\times$ trace height and raises some constraints' degree, converting free headroom directly into committed-data savings.
Do it by hand if your frontend will not.

**(e) Pick a richer degree-$D$ identity for the same price.** Take a 16-bit range check built as a set-membership product with exactly $D=9$ roots.
The degree is fixed at 9 by the budget, but *which* 9 roots is entirely free: $\{0,1,2,4,\dots,128\}$ gives a minimum trace length of 1024; $\{0,1,3,9,\dots,2187\}$ gives **64**.
...
**Within a bracket degree is completely free**: quotient chunks, LDE size, FFT size, opening count, two-adicity consumption and the blowup requirement are all identical.

**Practitioners converge on bracket tops.** Across the general-purpose systems surveyed, the declared budgets are 9, 9, 5, 3, 3 and (for a compiler that derives $D$ from the blowup $b$) $2b+1$ -- every one of them a $2^j+1$.

The exceptions are where the money is.
A Poseidon2 AIR configured `SBOX_DEGREE = 7, SBOX_REGISTERS = 0` reports degree 7 and therefore pays for **8 quotient chunks -- the same price as degree 9**.
Proven alone, two degrees are on the table for free: enough to fold an `is_real` selector and a booleanity check into the round constraints at zero marginal cost.
...
The AIR-side counterpart is a `max_constraint_degree()` hint: over-declaring is **allowed and silent**, with only a debug-assert that the hint is not too *small*.
...
**Degree buys permutation-trace width, one column per unit.** A published design rule of "at most 7 trace columns may be bussed in the same trace row" at $D=9$ is this same law, with the table side also on the bus.
A compiler that fuses bus terms automatically states the condition directly -- fuse only while `deg(combined_num) <= MAX_CONSTRAINT_DEGREE` and `deg(combined_den) <= MAX_CONSTRAINT_DEGREE - 1` -- so at $D=3$ with unit-degree fields **bus terms fuse exactly two-by-two**, and no amount of optimizer effort changes that.
...
At `max_degree = 2` it would be 14 intermediates, i.e. **48 extra wires per FRI-arity row.**
...
A single degree-8 gate forces $|G| = 1$ -- a private selector column.
Degree $\ge$ `max_degree` panics outright.
...
A system that works in **log**-space quantizes harder still: the budget is a power of two with no intermediate values at all.
In one such design every component declares `log_size() + 1` -- a uniform degree-2 budget, system-wide.
...
```
An S-box parameterised by `(SBOX_DEGREE, SBOX_REGISTERS)`: $(3,0)\to3$, $(5,0)\to5$, $(7,0)\to7$, and $(5,1)$, $(7,1)$, $(11,2)$ all $\to 3$.
...
```
The arithmetic: `when(f).assert_bool(c)` is $f\cdot c(c-1)$, degree $1 + 2\deg(c)$.
With $\deg c = 1$ that is 3, exactly the cap.
If $c$ were the flag-selected expression $f_{\text{add}}e_{\text{add}} + f_{\text{sub}}e_{\text{sub}}$ (degree 2), the constraint would be $1 + 2\cdot2 = \mathbf{5}$.

Price: $2\times$ the constraints, **no extra columns** -- the copies read the same witness.
**Constraint count and constraint degree are different currencies** (count enters soundness logarithmically, degree enters cost multiplicatively), and this trade sells the cheap one.
...
$R{=}1$: $2n\times 298 = 596n$. -> **low degree wins evaluation by $2.11\times$.**

**Break-even.** With $h$ = cost of committing one field element and $e$ = cost of one column-point of constraint work, high degree wins iff
...
Boundary selectors are non-zero at a single row, so they are degree-$(N-1)$ polynomials; the transition selector only needs to vanish on the last row and so is linear.

**`when_transition()` is free** -- absorbed by the $+1$ in $d(n-1)+1$.
...
When the body is at the cap, the only escape is to make the constraint true on **every** row, padding included, and pay in trace generation instead:
...
```
The padding-row filler has to solve for carries that make the *arithmetic* constraints hold on rows that mean nothing, which sometimes only works in the native field *because the padding values overflow the intended ranges* -- and the selector encoder must carry an extra slot for the dummy rows.
...
Price: 1 boolean column + 1 booleanity constraint per component.
Padding rows contribute zero to every LogUp sum because both the use and the yield carry weight 0.

A further refinement removes even that column, in the case where a padding row can be made to claim a *true* fact: let padding rows send a genuinely valid tuple such as `ADD(0,0) = (0,0)`, then cancel them all with **one degree-0 bus term per AIR instance**, weighted by an instance-level value equal to the padding count.
Zero columns, zero per-row constraints, one public-ish value.
...
Cost: $O(d\mu)$ proof elements and $O(d)$ prover work per round, against $O(2^{\lceil\log_2 d\rceil} n)$ **committed** field elements in a quotient prover.
HyperPlonk states it directly:
...
**Rule: budget by multiplication count, not degree.** $x^7$ as $((x^2)^2)\cdot x^2\cdot x$ costs 4 multiplications whether or not you split it, and splitting it into committed columns is now a **pure loss** -- commitment is the expensive part and you just added a column.
...
i.e. degree 1 is free, degree $\ge 2$ costs one commitment, and **degrees 2 through 100 cost the same commitment.** The budget question becomes "how many non-linear columns", not "how deep".
...
(i) The $d-1$ cross-term commitments are **MSMs over the full witness** -- $O(dn)$ *group* operations, and a group operation is ~100$\times$ a field multiplication.
Moving Nova ($d=2$, one cross term) to a degree-5 gate **quadruples the MSM bill**.
