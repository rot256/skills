...
A degree-$d$ constraint composed with degree-$(n-1)$ trace polynomials has degree $d(n-1)$ -- or $d(n-1)+1$ once the
linear transition selector is folded in.
Dividing by the vanishing polynomial $Z_H$ leaves $\approx (d-1)n$, which does not fit in one column of height $n$ and is
therefore committed in **chunks**, whose count is **rounded up to a power of two** because the chunk domains must be
cosets of a subgroup:

```rust
// Bound the degree of the quotient polynomial by constraint_degree - 1, then choose the
// number of quotient chunks as the smallest power of two >= (constraint_degree - 1).
log2_ceil_usize(constraint_degree - 1)
```

**Price.** $2^{\lceil\log_2(D-1)\rceil}$ chunks, each $E$ base-field columns of height $n$, all LDE'd, committed and
opened.
The same exponent appears in KZG-based PLONKish under a different name -- the *extended domain* is doubled until
`(1 << extended_k) >= n * (cs.degree() - 1)` -- and in provers that call the chunks "segment polynomials".
...
**Deployed configurations sit exactly on the cliff, at $D - 1 = \text{blowup}$.**
One shipping config pairs `rate_bits: 3` with `max_quotient_degree_factor: 8`, tuned so that
`max_fft_points = 1 << (degree_bits + max(rate_bits, log2_ceil(quotient_degree_factor)))` is a *tie*; another pairs
inverse rate 4 with 4 quotient chunks.
Tuning two numbers to agree is fragile; the robust form derives one from the other:
```rust
...
Every quotient chunk needs its own coset, so the reachable trace height is
$2^{\text{TWO\_ADICITY} - \lceil\log_2(D-1)\rceil}$; a careful toolchain computes that ceiling and enforces it before
proving rather than failing deep inside the prover.
...
Every quotient chunk is opened at $\zeta$ and shipped, and the verifier recombines them
through Lagrange coefficients over the split domains.
...
**Practitioners converge on bracket tops.** Across the general-purpose systems surveyed, the declared budgets are
9, 9, 5, 3, 3 and (for a compiler that derives $D$ from the blowup $b$) $2b+1$ -- every one of them a $2^j+1$.

The exceptions are where the money is. A Poseidon2 AIR configured `SBOX_DEGREE = 7, SBOX_REGISTERS = 0` reports degree 7
and therefore pays for **8 quotient chunks -- the same price as degree 9**. Proven alone, two degrees are on the table
for free: enough to fold an `is_real` selector and a booleanity check into the round constraints at zero marginal cost.
**Whenever a hand-written gadget's degree is not of the form $2^j+1$, there is free degree sitting on the table.**
...
> of I.2 takes over -- so check before assuming the brackets exist. Log-space multi-AIR systems quantize even harder
> (II.9).
...
Some frontends make this an API, precisely because reserving is free:
```rust
...
```
The computed `degree()` then takes a max with the reservation.
The AIR-side counterpart is a `max_constraint_degree()` hint: over-declaring is **allowed and silent**, with only a
debug-assert that the hint is not too *small*.

And the detector for the opposite mistake -- paying for degree nobody used -- is four lines:
```rust
match config.max_constraint_degree.cmp(&actual_max_constraint_degree) {
    Ordering::Greater => warn!("configured degree {} > degree any constraint uses {}", ...),
    Ordering::Less    => info!(...),
    Ordering::Equal   => {}
}
```
`Greater` should be a **warning**, not a note: it is a whole quotient bracket bought and left idle.
Wire this check into your own build if the toolchain does not.
...
**Three unrelated codebases state the same constant.** A KZG PLONKish permutation argument chunks at
`chunk_len = cs_degree - 2`, with the intent spelled out -- *"we will fit as many polynomials $p_i(X)$ as possible into
the required degree of the circuit, so the following will not affect the required degree"*.
A STARK prover documents *"from one row to the next, (max_degree - 2) bounds the number of accumulated values"*.
A third splits its partial products at `chunk_size = max_degree; n.div_ceil(chunk_size) - 1`.

**Worked.** A PLONKish config with 80 routed wires at $D=8$: $\lceil 80/8\rceil - 1 = 9$ partial-product columns,
...
Batching messages into one fraction column has a computable degree. With messages $i$ carrying multiplicity $m_i$ and
RLC-compressed field expression $e_i$, the fraction constraint
$\bigl(\prod_i e_i\bigr)\cdot f = \sum_i m_i \prod_{j\ne i} e_j$ has degree
...
A published design rule of "at most 7 trace columns may be bussed in the same trace row" at $D=9$ is this same law, with
the table side also on the bus.
A compiler that fuses bus terms automatically states the condition directly -- fuse only while
`deg(combined_num) <= MAX_CONSTRAINT_DEGREE` and `deg(combined_den) <= MAX_CONSTRAINT_DEGREE - 1` -- so at $D=3$ with
unit-degree fields **bus terms fuse exactly two-by-two**, and no amount of optimizer effort changes that.
...
The rule the *interaction* obeys is usually **not** the rule the identities obey, and it is rarely documented next to
the degree constant. The four shapes seen in practice:

| interaction budget | escape |
|---|---|
...
| the LogUp formula of II.4 | flagged / mutually-exclusive form |

The affine-only rule is the strictest and the most consequential: a message field like `is_real * addr` is *illegal*, and
you must commit it. That converts a degree question into a width question **unconditionally**, which is why machines
built on it come out wide and shallow.
The separate bus budget is one **lower** than the identity budget for exactly the reason in II.4 -- a frontend using a
single budget for both would happily emit unprovable AIRs.
...
```
That is a *floor*, not a ceiling. A chip author who keeps a constraint at degree 2 "to be safe" on a chip that already
sends or receives is spending an intermediate column for literally nothing.

**(c) Widen accumulator and bus chunks to $D-2$ / $D-1$ per column** (II.3, II.4).

**(d) Inline intermediate columns away until the budget is full.** This is a whole class of constraint-system optimizer
in one sentence:
```rust
...
```
Each inline deletes one column $\times$ trace height and raises some constraints' degree, converting free headroom
directly into committed-data savings. Do it by hand if your frontend will not.

**(e) Pick a richer degree-$D$ identity for the same price.**
Take a 16-bit range check built as a set-membership product with exactly $D=9$ roots. The degree is fixed at 9 by the
budget, but *which* 9 roots is entirely free:
$\{0,1,2,4,\dots,128\}$ gives a minimum trace length of 1024; $\{0,1,3,9,\dots,2187\}$ gives **64**.
Same degree, same columns, **16$\times$ better**.
...
degree as far as the same wire count permits** -- because degree below the cap is *not* free in selector-group
...
$|G| = 1$ -- a private selector column. Degree $\ge$ `max_degree` panics outright.
...
```
"The whole system pays for the maximum", in five lines.
A system that works in **log**-space quantizes harder still: the budget is a power of two with no intermediate values at
all. In one such design every component declares `log_size() + 1` -- a uniform degree-2 budget, system-wide.

> **Where it stops.** One component asking for degree 3 moves the global bound to `log_size + 2`, doubling the
> composition-evaluation domain for the *entire* machine -- dozens of components, hundreds of columns. That is why a
> cubing operation ends up as its own component with an intermediate column rather than as a cubic constraint inline.
...
(7, 1) => { builder.assert_eq(committed_x3, x.cube());  // degree 3
            committed_x3.square() * x }                 // degree 3, 1 column
(11, 2) => { /* two committed powers, degree 3 */ }
```
An S-box parameterised by `(SBOX_DEGREE, SBOX_REGISTERS)`:
$(3,0)\to3$, $(5,0)\to5$, $(7,0)\to7$, and $(5,1)$, $(7,1)$, $(11,2)$ all $\to 3$.
Price: exactly `SBOX_REGISTERS` committed columns per S-box.
...
For a Poseidon2-based Merkle tree over a 31-bit field that ratio is comfortably above 1 -- which is why reference
Poseidon2 AIRs default to the register-free, high-degree form. For a Keccak/Blake3 tree on a machine with vectorized
field arithmetic it can flip.

**The decisive term is not in that table.** A *VM* integrating the same AIR picks $R=1$ anyway, because at $D=7$ the
required blowup goes from $2^1$ to $2^3$ (I.2), multiplying the committed cells of **every chip in the machine** by 4:
```rust
...
## III.3 Worked break-even #2 -- PLONKish permutation chunking
...
> Past a handful, committing a selected intermediate column wins. The companion move -- deliberately keeping a selector
> *affine* so it can be multiplied into anything -- is worth writing down as a comment next to the column definition,
> because the next person to add a term to it will not otherwise know it is load-bearing.
---
...
```
and the three built-in selectors are **not** priced alike:
...
Boundary selectors are non-zero at a single row, so they are degree-$(N-1)$ polynomials; the transition selector only
needs to vanish on the last row and so is linear.
...
A running-sum decomposition checks each $K$-bit window with a product over $2^K$ roots. Gated by a selector, that is a
`selector * range_check(word, range)` expression of degree `range + 1`, so $2^K$ must be at most `degree_bound - 1` for
the range-check constraint to stay inside the budget. Production gadgets enforce it as a hard
...
> Production lookup range-check gadgets do 10 bits per row where the product form does 3 -- a **3.3$\times$ row saving**
> for three extra columns amortized over the whole circuit.
...
```
The padding-row filler has to solve for carries that make the *arithmetic* constraints hold on rows that mean nothing,
which sometimes only works in the native field *because the padding values overflow the intended ranges* -- and the
selector encoder must carry an extra slot for the dummy rows.
That is the whole trade, and it is what took a production SHA-2 AIR family from **degree 4 to degree 3**.
...
The cleanest resolution of IV.3's dilemma. Rather than zero-filling -- which violates the arithmetic constraints and
therefore forces *every* constraint to be gated -- duplicate the first real input and zero only the enabler:
...
A further refinement removes even that column, in the case where a padding row can be made to claim a *true* fact:
let padding rows send a genuinely valid tuple such as `ADD(0,0) = (0,0)`, then cancel them all with **one degree-0 bus
term per AIR instance**, weighted by an instance-level value equal to the padding count. Zero columns, zero per-row
constraints, one public-ish value.

> **Where it stops.** A padded row must genuinely satisfy every ungated constraint -- which is why *copying a real row is
> mandatory* and why a component with **zero** real rows cannot exist -- there is no real row to copy.
> The bus-cancellation variant needs the padding row's claim to be true *and* the cancelling term to be exactly
> countable.
...
the prover sends in each of the $\mu = \log n$ rounds -- the verifier literally reads `max_degree` scalars per round
(`partially_verify_sumcheck_proof(.., MAX_CONSTRAINT_DEGREE + 1)`).
...
> than $ds$ in Plonk."* and *"the additional arithmetic work that the prover needs to do depends on the number of
...
Some multilinear systems draw the free/paid line at degree **1** rather than at a cap:
...
> Skipping and high degree do not compose -- pick one.
...
Sangria states it plainly: *"cross terms will now have powers of $r$ from 1 to $d-1$ ... the prover will perform
$O(d(n+s))$ field operations and $O(d(n+s))$ point additions."*
...
1. **Compute your system's actual max degree symbolically before committing to anything.**
   Every frontend can do it -- a symbolic AIR evaluator, a `degree()` on the constraint system, an expression evaluator
   plus a degree bound. Guessing here invalidates steps 2--6.
