...
A worked instance: a 256-bit field in **88-bit limbs** -> 3 cells, FF-mul in 2 rows, each limb range-checked -- and that 88 is *derived* from a wire budget, not chosen (`plonkish.md` III.2).
...
- **Spread / interleave** (as used by SHA-256 gadgets): map a k-bit word to one with a 0 between every bit.
  ...
  Needs a shared 2^16 table. The *diluted form* is the same idea with variable spacing, and gets AND/XOR/OR from degree-**1** constraints with no bit columns at all.
