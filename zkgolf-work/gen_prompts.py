...
WORKQUEUE={
 ...
 "sha256-hash": (144985, None, [
  ...
  ("0 — THE -6 AT 144,321 IS 0-FOR-4 AND THE SUSPECT IS NOW A SINGLE STRICT INEQUALITY. CHECK IT BEFORE YOU "
   "WRITE ANYTHING","*** FOUR INDEPENDENT TREES REACHED 144,321 = 71,872 alloc + 72,449 constr AND ALL FOUR WERE "
   "JUDGED AND REJECTED (1ee0c2b0, bad91b20, 635fa2cf, 252a1a6f). *** They are 12-19 files apart from one another, "
   "so genuinely different implementations, but intersecting their diffs against the seed leaves only four common "
   "files, two of which are cost bookkeeping. The shared substantive edit swaps the wide round family for a W4 "
   "variant and WEAKENS the state assumption on words 3 and 7 from RPShared.Numeric5W to Numeric4W. *** THE "
   "FOURTH TREE SHIPS THE ARGUMENT IN WRITING, IN A NEW FILE Numeric4W.lean THAT IS NOT IN THE SEED. *** The "
   "definition is `structure Numeric4W (w : Vector (F p) 32) : Prop where bound : valueBits w < 4 * 2^32` — "
   "A STRICT INEQUALITY — and the docstring argues: 'the deferred state words 3 and 7 grow by at most one carry "
   "per block, so after k compressions they are bounded by (k+1) * 2^32. Blocks 2, 3 and 4 therefore receive words "
   "bounded by 2*2^32, 3*2^32 and 4*2^32 respectively — all of them Numeric4W.' *** SPOT THE BOUNDARY: Numeric4W "
   "DEMANDS STRICTLY LESS THAN 4*2^32, AND THE ARGUMENT GIVES BLOCK 4 A WORD BOUNDED BY 4*2^32. *** If that growth "
   "bound is non-strict the claim fails exactly at the last interior block, which would explain four identical "
   "rejections; if it is strict everywhere the argument stands and the defect is elsewhere. THAT IS ONE LINE OF "
   "ARITHMETIC AND IT NEEDS NO CERTIFICATE — settle it first. *** THE TWO HALVES ARE INSEPARABLE: *** the same "
   "docstring says the head-room 'is exactly what pays for the truncated upper-Sigma0 gadget in the first round "
   "pair of those blocks (31 lanes instead of 32)', so the -6 IS the 31-lane Sigma0 and you cannot keep it while "
   "reverting to Numeric5W. *** NOT CONDEMNED: THE SCORE. *** 144,321 by a route that does not weaken this bound "
   "is untouched, and four rejections is inside the range where persistence has paid here — variable-base 303,328 "
   "took five trees, fixed-base stageO2 took seven."),
