...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (47435, 34030, [
  ...
  ("0 — THE -1 AT 47,413 IS SETTLED NEGATIVE, 0-FOR-3, AND tick.py NOW BLOCKS IT","*** DO NOT READ THIS AS "
   ...
   "THE SUBSTITUTED EXPRESSION, which excludes no environments and is why the verifier threw them out. *** 0-FOR-6, AND THE SIXTH ATTEMPT MOVES THE PROBLEM OFF THE SITE "
   "AND ONTO THE ARGUMENT. DO NOT PICK A SITE AND WRITE A CERTIFICATE — READ THIS FIRST. *** Five attempts flipped "
   "PairAddFold.stageO2_scw. The sixth ALSO flipped a SECOND site, PairAddFoldA.stageAF (PairAddFoldA.lean:129, "
   "the five-position gfFold5/vFoldAL layout, <425,428> -> <425,427>), and failed identically. TWO DIFFERENT "
   "SITES, SAME OUTCOME — so the site is not the invariant. *** WHAT ALL SIX SHARE IS ONE SENTENCE, AND IT IS THE "
   "DEFECT. *** Every attempt justifies the deletion the way FoldLinIdent5's own docstring does: 'with the quotient "
   "written as the affine expression qInv5 = P256^-1 (lhsPoly5 - tailPoly4), the native row HOLDS IDENTICALLY IN "
   "THE WIRES, so circuitNoTop applies and the row can be deleted'. That is redundancy GIVEN THE SUBSTITUTION. It "
   "excludes no environments, and our obligation is unconditional over every environment satisfying the REMAINING "
   "constraints. The verifier has now rejected that sentence six times. *** WHAT THE WORKING VERSION SAYS INSTEAD. "
   "*** A competitor banked this exact family on variable-base, and their argument is different in kind: the "
   "quotient is recovered as an affine expression AND ITS RANGE CHECK IS RETAINED, and the native row is deleted "
   "because *** THE CARRY LOOP ALREADY PROVES EQUALITY OVER THE BOUNDED INTEGER REPRESENTATIVES *** — the identity "
   "moves into the formal assumptions because something ELSE pins it. *** SO DO NOT START FROM A SITE. *** Start "
   "from that invariant and ask WHICH OF OUR FOLD SITES HAS A CARRY LOOP PINNING THE BOUNDED INTEGER EQUALITY. If "
   "one does, that is where this lands. If none does, the technique does not transfer to fixed-base and the "
   "direction is closed — say so, because after six rejections that is a result worth having and it frees every "
   "future slot on this slug. Two facts to save you re-deriving them: stageO2 ALREADY has the quotient range check "
   "(ChainFold.lean:40-41, RangeCheck.circuit 67 on crtQInv, identical to stageX's at :307-308), so a missing "
   "range check is NOT the problem; and stageX differs from stageO2 in that every one of its inputs appears in the "
   "fold, whereas stageO2's yA and yT1 appear nowhere in it."),
