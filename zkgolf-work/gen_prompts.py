...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (47435, 34030, [
  ("0 — THE -1 AT 47,413 IS SETTLED NEGATIVE, 0-FOR-3, AND tick.py NOW BLOCKS IT","*** DO NOT READ THIS AS "
   ...
   "sorry, computableWitness present throughout. *** IT HAS NOW BEEN REJECTED THREE TIMES OUT OF THREE (db0c21da, "
   "7ab899af, fd50107d) UNDER THREE DIFFERENT PROOF ROUTES (FoldLinIdent5, FoldQInvW, FoldQInvC1), AND tick.py "
   "NOW REFUSES TO SUBMIT IT. *** You will not get this one past the pre-submission guard, so spend the slot "
   "elsewhere unless you can first answer the question below. *** THE REAL SIGNAL IS IN THE WORKING TREE: the "
   "verified 47,414 applies NoTop at ChainFold.stageX_scw (ChainFoldCW.lean:240, using ll_effNT at :253) and "
   "deliberately does NOT apply it at PairAddFold.stageO2_scw (:62), even though the lemma it would need is "
   "sitting in the same file. Two sibling theorems, identical fold parameters, opposite treatment, in a tree "
   "that verifies. Whatever distinguishes stageO2 from stageX IS the question. Establish that before anything "
   "else: show what the deleted top row supplies at stageO2 that it does not supply at stageX, and show the "
   "fold's quotient bound SURVIVES WITHOUT THE ROW, not that the row is redundant GIVEN the affine quotient. An identity true by "
   "construction of the expression you substituted excludes no environments. *** CONDEMN THE SITE, NOT THE "
   "NUMBER: 47,413 is reachable by other means and nothing here rules those out. Deleting the quotient WIRE "
   "still works; it is deleting the ROW at this fold that fails."),
