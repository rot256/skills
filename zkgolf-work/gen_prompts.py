...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (50355, 49536, [
 ...
 "secp256k1-scalar-mul": (316436, None, [
  ...
  ("762 CONFIRMED, PURE NUMERAL WORK","TIGHTEN THE DERIVED WIDTHS IN THE FOLD FAMILY — NO SOUNDNESS OBLIGATION, ONE COMPLETENESS BOUND PER INSTANTIATION, THE MOST LANDABLE ITEM ON THIS LIST. MulModFold, MulModFoldT and MulModFold32 take the operand caps Ca and Cb as PARAMETERS but hard-code the DERIVED quotient and carry widths at the widest admissible product, so tighter call sites overpay. MEASURED: MulModFold32 at Ca = Cb = 2^32 is FOUR TIMES under cap, so qBitsFold32 39 -> 37 and wfFold32 48 -> 46, worth 8 per site x ~77 sites = 616; MulModFold at (2^64, 2^64) = 80 over 10 sites; MulModFold at (2^64, 3*2^64) = 44 over 11 sites; GLVStepLastSlope.lean:143 and :145 = 16, easy since each ifFalse operand is provably already zero; PointValid fold widths = 6. ONE MORE CANDIDATE IN THE FAMILY: MulModFoldT at DivOrZeroN32 overstates its target cap Nt by ~66 bits, worth 126."),
