...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (49218, 49536, [
  ("survival, not score — read this before you optimise anything here","ELABORATION TIME IS A FIRST-CLASS COST ON "
   "THIS SLUG AND IT IS NOW BLOCKING THE NEXT RECORD. THREE INDEPENDENT TREES HAVE REACHED EXACTLY 47,417 AND ALL THREE FAILED — two verifier timeouts and one clean failure — so whatever takes this tree from 47,437 to 47,417 pushes it past what the verifier will elaborate. DO NOT AIM AT THAT DESIGN AGAIN. Either reach a lower score by a structurally different route, or spend this slot BUYING ELABORATION HEADROOM FIRST: mark the heavy definitions irreducible — cost constants, params tables, and any def whose body is a large literal vector — of which only 48 of 202 solution files do so today. AND ATTACK THE `decide` SITES, WHICH ARE THE BIGGER TARGET HERE: this tree carries 1,626 plain `by decide` (heaviest file EvenTableTop.lean at 132) against only 34 uses of `decide +kernel` across ALL EIGHT TREES. `decide +kernel` evaluates the decidability instance IN THE KERNEL instead of building the term elaborator-side. Also note the toolchain: we are pinned to v4.28.0, and Lean 4.32.0 is where bounded-quantifier Decidable instances (Nat.decidableBallLT and friends) stopped taking QUADRATIC time — we cannot upgrade, so any `by decide` on a 'forall k < N' with large N is quadratic here and should become an explicit conjunction or a chunked decide, the way the RSA tree already proves tripleCount16 in twelve chunks. DO NOT reach for set_option debug.skipKernelTC: it disables kernel typechecking and will not survive the config check. A reduction that cannot be verified banks nothing and burns the slot. All three verifier timeouts in the loop's entire history are on "
 ...
 "sha256-hash": (144985, None, [
  ("survival first — BUT WE HAVE NOW BANKED ONE, AND IT SETTLES THE WALL QUESTION","*** THE FULL SUBMISSION LEDGER ON THIS SLUG, AND IT SETTLES THE WALL QUESTION AGAINST THE OBVIOUS READING. -612 TIMED OUT. -306 TIMED OUT. -298 VERIFIED AND TOOK THE RECORD (144,687, from yash1io's 144,985). -280 TIMED OUT TWICE. TWO FACTS, AND THE SECOND IS THE ACTIONABLE ONE. (1) SIZE DOES NOT PREDICT THE OUTCOME: the SMALLEST reduction we have attempted here timed out while a LARGER one verified, so AIMING SMALLER IS NOT A STRATEGY and has now cost four attempts. (2) EVERY SINGLE FAILURE ON THIS SLUG IS A VERIFIER TIMEOUT — not one soundness failure, not one bad ledger. So the binding constraint is ELABORATION TIME AND NOTHING ELSE, and the -280 at 144,389 IS A SOUND SOLUTION THAT SIMPLY WILL NOT ELABORATE IN BUDGET. *** YOU WILL PROBABLY FIND A -280 THAT LANDS AT 144,389. IT IS CORRECT, IT IS SOUND, AND IT HAS TIMED OUT FOUR TIMES. READ THE NEXT PARAGRAPH BEFORE YOU START WRITING. *** Four separate jobs seeded from this same tree have found that identical -280 and every one blew the ELABORATION budget — never a soundness failure, never a bad ledger. The cost is intrinsic to the change, not bad luck in one proof, and the fourth job had this warning in front of it and produced 144,389 anyway. So this is NOT an instruction to avoid it: it is the dominant reduction from this seed and you are likely to rediscover it no matter what. IT IS AN INSTRUCTION ABOUT HOW TO SPEND THE SLOT. BUDGET AT LEAST HALF YOUR EFFORT ON PROOF COST, NOT CIRCUIT SIZE. Concretely, and all of it must be in THIS submission because a cost-neutral tidy-up cannot be submitted at all: mark every heavy definition `irreducible`; convert `by decide` sites to `decide +kernel` so instances evaluate kernel-side instead of elaborator-side; keep new lemmas small and avoid anything that forces re-elaboration of the cost proofs; and if you must choose between another 20 points of circuit and a materially cheaper proof, TAKE THE CHEAPER PROOF. A verified -200 is worth infinitely more than a fifth timed-out -280. IF YOU FINISH THE HEADROOM WORK AND STILL HAVE BUDGET, take a DIFFERENT reduction rather than deepening this one — a -298 verified here where this -280 has failed four times, so the SHAPE of the change decides the outcome, not its size. *** BUY ELABORATION HEADROOM BEFORE YOU REDUCE ANY "
   ...
   "definitions irreducible, and convert `by decide` sites to `decide +kernel` so the instance is evaluated in the kernel rather than elaborator-side (only 34 sites across all eight trees do this today). Targets: cost constants, the big params and schedule tables, any def whose body is a large "
  ...
  ("MEASURE FIRST, UNKNOWN YIELD — THE ONE BLOCK IN THE TREE WHOSE ROWS ARE UNEXPLAINED","RUN THE "
   "ROW-DELETION CRITERION ON THE PADDING PREDICATE. Re-derived against the 144687 seed: every `def X : Count` "
   "in Solution/SHA256 ranked by constraints-minus-allocations gives checkPad7Cost <0,2167> (Cost7.lean), "
   "checkPad7SparseCost <0,2063> and checkPad7SparseUncheckedCost <0,2062> (CostSparse.lean) — and then the "
   "NEXT LARGEST GAP IN THE ENTIRE TREE IS 32 (selectDigestCost <8,40>). Every row introduces at most one "
   "witness, so r = m is the structural floor and the whole circuit sits within 32 of it EXCEPT ~2062 rows, "
   "1.4% of the score, that ALLOCATE NOTHING. When this was first measured at the 145470 seed the runner-up gap "
   "was 88; it is now 32, so every improvement since came from elsewhere and this block has never been touched. "
   "THE METHOD: expand each row into its quadratic-monomial coefficient vector plus linear part and solve "
   "sum_k lambda_k Q_k[i,j] = 0 by plain Gaussian elimination. Rows with lambda_k != 0 are deletable; otherwise "
   "you harvest a free linear constraint. Measured yield is ~0% on hand-tight circuits and 20%+ on mechanically "
   "generated ones. *** DO NOT ASSUME THE SECOND FIGURE APPLIES HERE -- THE FIRST ONE NAMES THIS CIRCUIT. "
   "The row-deletion criterion found 32 redundant rows out of 30,134 on circomlib SHA-256 (0.1%), and the "
   "Distilling Constraints authors state that circuits using bitwise operations leave small room for "
   "optimization. SHA-256 is the named negative example in both. What may still distinguish THIS block is that "
   "both measurements cover a whole SHA-256 circuit, overwhelmingly bitwise gadget rows -- the shape our own "
   "r-versus-m survey confirms is tight here -- and neither isolates a pure assertion block. So the honest "
   "position is that the yield is UNKNOWN. *** YOUR FIRST AND POSSIBLY ONLY DELIVERABLE IS THE MEASUREMENT: run the elimination over the ~2062 rows and "
   "report the rank deficiency. It is cheap and it settles the question either way. Do not rewrite anything "
   "before that number exists; a confirmed zero is a real result, because it closes the last structurally "
   "unexplained block in the tree. "
   "*** STATE THE CAVEAT HONESTLY IN YOUR SUMMARY: this is NOT a proof that 2062 is above its own floor. The "
   "rows-above-allocations argument concerns rows needed to DETERMINE witnesses, and an assertion block "
   "determines none. It is the one place whose row count has no witness-pinning explanation, which is a reason "
   "to attack it, not a guarantee. Do not quote a point estimate until the elimination has been run. ***"),
