...
WORKQUEUE={
 ...
 "sha256-hash": (144985, None, [
  ("survival first — BUT WE HAVE NOW BANKED ONE, AND IT SETTLES THE WALL QUESTION","*** THE FULL SUBMISSION LEDGER ON THIS SLUG, AND IT SETTLES THE WALL QUESTION AGAINST THE OBVIOUS READING. -612 TIMED OUT. -306 TIMED OUT. -298 VERIFIED AND TOOK THE RECORD (144,687, from yash1io's 144,985). -280 TIMED OUT TWICE. TWO FACTS, AND THE SECOND IS THE ACTIONABLE ONE. (1) SIZE DOES NOT PREDICT THE OUTCOME: the SMALLEST reduction we have attempted here timed out while a LARGER one verified, so AIMING SMALLER IS NOT A STRATEGY and has now cost four attempts. (2) EVERY SINGLE FAILURE ON THIS SLUG IS A VERIFIER TIMEOUT — not one soundness failure, not one bad ledger. So the binding constraint is ELABORATION TIME AND NOTHING ELSE, and the -280 at 144,389 IS A SOUND SOLUTION THAT SIMPLY WILL NOT ELABORATE IN BUDGET. *** AND 144,389 HAS NOW TIMED OUT THREE TIMES FROM THREE SEPARATE JOBS. Do not simply resubmit it. *** Three jobs seeded from the same 144,669 tree all found the same -280 and all three blew the elaboration budget, which says the cost is INTRINSIC TO THAT PARTICULAR CHANGE rather than bad luck in one proof. Note this is the RE-DERIVATION PATTERN FAILING: normally an independent tree reaching the same score verifies, and here it did not, three times. So treat 144,389-by-the-obvious-route as a KNOWN TRAP. TWO WAYS PAST IT, PICK ONE DELIBERATELY. (a) TAKE THE SAME -280 BUT BUDGET FOR THE PROOF FROM THE START: mark heavy definitions irreducible, convert `by decide` sites to `decide +kernel` so instances evaluate kernel-side rather than elaborator-side, and keep the new lemmas small — the headroom work must be IN THE SAME SUBMISSION, since a cost-neutral tidy-up cannot be submitted at all. (b) FIND A DIFFERENT REDUCTION. A -298 verified here and a -280 has failed three times, so the shape of the change decides this, not its size. If you catch yourself re-deriving 144,389 by the obvious route, stop and take a different target. *** BUY ELABORATION HEADROOM BEFORE YOU REDUCE ANY "
