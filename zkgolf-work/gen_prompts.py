...
WORKQUEUE={
 ...
 "sha256-hash": (144985, None, [
  ("survival first — BUT WE HAVE NOW BANKED ONE, AND IT SETTLES THE WALL QUESTION","*** THE FULL SUBMISSION LEDGER ON THIS SLUG, AND IT SETTLES THE WALL QUESTION AGAINST THE OBVIOUS READING. -612 TIMED OUT. -306 TIMED OUT. -298 VERIFIED AND TOOK THE RECORD (144,687, from yash1io's 144,985). -280 TIMED OUT TWICE. TWO FACTS, AND THE SECOND IS THE ACTIONABLE ONE. (1) SIZE DOES NOT PREDICT THE OUTCOME: the SMALLEST reduction we have attempted here timed out while a LARGER one verified, so AIMING SMALLER IS NOT A STRATEGY and has now cost four attempts. (2) EVERY SINGLE FAILURE ON THIS SLUG IS A VERIFIER TIMEOUT — not one soundness failure, not one bad ledger. So the binding constraint is ELABORATION TIME AND NOTHING ELSE, and the -280 at 144,389 IS A SOUND SOLUTION THAT SIMPLY WILL NOT ELABORATE IN BUDGET. *** THE VERIFIER TIMEOUT IS PARTLY STOCHASTIC. RESUBMIT AN IDENTICAL SOLUTION RATHER THAN ABANDONING IT. *** Measured on this slug: the SAME -280 landing at 144,389 was submitted FIVE times from five jobs. Four TIMED OUT and the fifth VERIFIED AND TOOK THE RECORD. Identical score, identical reduction, opposite outcomes — so a timeout is evidence about that RUN, not proof that the solution is too expensive. The full ledger here is -612 timed out, -306 timed out, -298 verified, -280 timed out four times then verified: FOUR-FIFTHS OF OUR FAILURES ON THIS SLUG WERE RECOVERABLE BY RETRYING, and NOT ONE failure was ever a soundness error. WHAT THAT MEANS FOR YOUR SLOT. (1) If you land on a reduction and it feels expensive to prove, SUBMIT IT ANYWAY — do not trade it down for a safer one on the strength of a timeout you are anticipating. (2) Still buy headroom in the SAME submission, because it raises the per-run success probability and a cost-neutral tidy-up cannot be submitted on its own: mark heavy definitions `irreducible`, convert `by decide` to `decide +kernel` so instances evaluate kernel-side, keep new lemmas small. (3) DO NOT AIM SMALLER TO DODGE THE WALL — the SMALLEST reduction attempted here (-280) timed out four times while a LARGER one (-298) verified first try. Size does not predict the outcome; proof shape and luck do. BUY ELABORATION HEADROOM BEFORE YOU REDUCE ANY "
