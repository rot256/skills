...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (49218, 49536, [
  ...
  ("372 — BUILD IT AND REPORT THE ERROR; TWO IMPLEMENTATIONS HAVE FAILED","THE CRT FOLD MULTIPLY. Nothing ever "
   "consumes interpolatedMul's seven convolution coefficients — every consumer reads foldLhs, i.e. the product "
   "mod (X^4 - cF) — and cF = 2^32 + 977 IS a fourth power mod r, so the residue ring splits completely and the "
   "Winograd rank is 2N - k = 4. THE ALGEBRA IS VERIFIED AND IS NOT IN DOUBT: the roots "
   ...
   "15481058158945279400336798038226894666879721275580208710828625798757467132996 and "
   "15865747286151745787645593793184946820902319848935289103006992069981740477241 are distinct, below r, and "
   "each satisfies rho^4 = cF; r - 1 has 2-adicity 28. GADGET: witness F : Vector _ 4 and for each root emit ONE "
   "row (SUM a_i rho_j^i) * (SUM b_i rho_j^i) = SUM_k F_k rho_j^k — one product, all coefficients compile-time "
   "constants; foldLhs becomes F[k] + wExp v k, still affine, and nfFoldL / nfFoldWL / offFold* / wfFold* are "
   "UNCHANGED. Worth -374 measured: extCost <865,869> -> <856,860>, i.e. -9 for the three convolutions per ext "
   "stage. *** BUT TWO INDEPENDENT JOBS HAVE ALREADY BUILT THIS AND BOTH SUBMISSIONS FAILED, as did a repair "
   "that corrected the first one's stale Main.lean literals. Neither tree contained `sorry`; the second had "
   "Main.lean and combCost in agreement. costIs_crtFoldMul <4,4> and the isR1CS chain via isR1CSRow_mul_sub both "
   "appear correct. SO YOUR FIRST TASK IS DIAGNOSTIC, NOT CREATIVE: BUILD IT AND REPORT THE ACTUAL LEAN ERROR "
   "VERBATIM IN YOUR SUMMARY, even if you cannot fix it — that error message is worth more to us than the 374. "
   "The most likely places are the transfer of `.val` bounds through foldLhs_lt once the seven convolution "
   "coefficients become four folded witnesses, and the completeness direction where the prover must set F_k := "
   "z_k + cF*z_{k+4}. IF YOU CANNOT MAKE IT BUILD, SAY SO PLAINLY AND SHIP A DIFFERENT ITEM — do not spend the "
   "whole job on it. ***"),
