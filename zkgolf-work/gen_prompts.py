...
WORKQUEUE={
 ...
 "sha256-hash": (144985, None, [
  ("READ THIS FIRST. TWO OF THE FOUR KILLER LINES ARE A COST-LITERAL DESYNC THAT *YOU* INTRODUCE, AND THE "
   "ROOT CAUSE IS NOW MEASURED, NOT GUESSED","*** THE VERIFIER LOG IS READ AUTOMATICALLY ON EVERY FAILURE AND "
   ...
   "*** 568/608 ARE NOT A SIZE PROBLEM. THEY ARE A FALSE PROOF, AND LEAN IS BURNING THE WHOLE BUDGET REFUTING "
   "IT. *** TWO FILES HAVE TO AGREE ON THE SAME COST LITERALS AND THE FAILING TREES DO NOT:\n"
   "  * `R1CSCompressorsD.lean`, `lemma block5D_output_layout` — states `CompressBlock5DPack.circuit.output "
   "input n = #v[fresh32D n <k0>, fresh32D n <k1>, ...]` and is PROVED BY `simp only [fresh32D, circuit_norm, "
   "...]`, so ITS numbers are dictated by the circuit and move automatically when you change costs.\n"
   "  * `ComputableDCompressorsFull.lean`, `theorem output_eval_eq_of_agreesBelow` (line 568) — HAND-ASSERTS "
   "those same numbers in eight `· exact fresh32D_eval_eq_of_agreesBelow offset <k_i> <limit> ...` bullets, "
   "plus the `hagree : env.AgreesBelow (offset + <limit>)` binder. These DO NOT move automatically.\n"
   "*** MEASURED ACROSS THE ACTUAL FAILING TREES: *** the two that died had layout 10329/10434/10466 but "
   "bullets 10332/10437/10469 — OFF BY EXACTLY +3, with `limit` also +3. The trees whose numbers agreed "
   "compiled that theorem in about ONE SECOND. *** MECHANISM, CONFIRMED WITH `set_option diagnostics true` ON "
   "A STANDALONE v4.28.0 REPRO: *** `isDefEq (fresh32D n 10329) (fresh32D n 10332)` delta-unfolds both, tries "
   "the `f a =?= f b` congruence heuristic on `Array.ofFn.go`, FAILS because the offsets differ, and then "
   "falls back to full `whnf` of both sides — `Array.push`/`Array.toList`/`List.concat`/`List.rec`, about "
   "487,000 unfoldings for ONE 32-lane vector pair. `Nat.add` unfolds 38 times, so THE NUMERAL BEING LARGE IS "
   "IRRELEVANT; what costs is that a MISMATCH pushes isDefEq onto the reducing branch. The signature is "
   "diagnostic: a `whnf` timeout at the `exact` (col 10) AND at the `theorem` (col 0), both at the DEFAULT "
   "200000 — exactly 608:10 and 568:0. A wrong `limit` alone is refuted instantly with a clean `Application "
   "type mismatch`; it is the wrong `k` that reduces. *** RAISING maxHeartbeats IS QUANTITATIVELY FATAL HERE, "
   "MEASURED ON THE REPRO: *** 200,000 -> whnf timeout at 23.8 s; 800,000 -> whnf timeout at 89.9 s; "
   "2,000,000 -> whnf timeout at 242.4 s. LINEAR, AND IT NEVER SUCCEEDS. Eight bullets at 2M is about 1900 s, "
   "past the HARD 1200-SECOND WALL CLOCK — which is precisely how a clean heartbeat error becomes a wall-clock "
   "kill. DO NOT TOUCH maxHeartbeats. *** THE FIX, IN THIS ORDER. *** (1) MAKE THE PROOF CARRY NO COST "
   "LITERAL AT ALL — this is the real fix, because it makes the theorem immune to every future cost change. "
   "The signature is `fresh32D_eval_eq_of_agreesBelow (n k limit : ℕ) ... (hk : k + 32 ≤ limit)` concluding "
   "about `fresh32D n k`, so `k` IS DETERMINED BY THE GOAL and you can simply write `_`:\n"
   "      all_goals exact fresh32D_eval_eq_of_agreesBelow offset _ <limit> env env' hagree (by omega)\n"
   "  Verified against three different simulated cost layouts (shift 0, +3, -105): the identical proof text "
   "compiled unchanged every time, 0.79-1.46 s. Only `limit` remains a literal, and you can state that as the "
   "cost constant itself rather than a numeral. (2) `attribute [irreducible] fresh32D` — MEASURED PURE WIN: "
   "correct proofs still compile AND GET FASTER (0.62 s vs 1.05 s), wrong ones fail in 1.35 s with a readable "
   "`Application type mismatch` instead of eating 200k heartbeats. It does NOT break `block5D_output_layout`, "
   "because `simp only [fresh32D]` fires via EQUATION LEMMAS, not delta reduction. Clean itself uses this "
   "idiom (`attribute [irreducible] Eval.eval`). (3) IF YOU CHANGE COST CONSTANTS AND KEEP THE HAND-WRITTEN "
   "NUMBERS, GREP BOTH FILES AND MAKE THEM AGREE BEFORE SUBMITTING — this single check would have saved three "
   "queue slots. *** 498/528 ARE A DIFFERENT AND GENUINE PROBLEM — do not assume the above fixes them. *** "
   "Their offsets are NOT desynced, and 528 is a `tactic execution` timeout, not `whnf`, which is simp cost. "
   "498 is `simpa only [circuit_norm, ...]` and 528 is a bare `simp [FormalCircuit.toSubcircuit, "
   "DeferAdd.circuit, DeferAdd.main, ...]`. Stop unfolding `main`/`toSubcircuit` inside simp; instead add "
   "`@[circuit_norm] lemma circuit_localLength (x) : circuit.localLength x = <numeral> := rfl` AT BOTH "
   "SPELLINGS (`elaborated.localLength` and `circuit.localLength`), proved once at a SYMBOLIC offset. "
   "sp1-lean reports one chip going 243 s -> 1.7 s from exactly this, with zero heartbeat bumps. (This half "
   "is reasoned from Clean's `doc/performance-problems.md` — which is on `main`, NOT at our pinned commit — "
   "and from sp1-lean, and is NOT measured, unlike the 568/608 fix which was reproduced and fixed end to "
   "end.) *** WHAT THIS DOES AND DOES NOT MEAN. *** It is NOT soundness — the verifier never reaches the "
   "soundness question, so nothing that died here (W4, the six-lane deferral) has been refuted; they are "
   "UNAFFORDABLE, not wrong. It is NOT the row count. *** WRITE OFFSETS THE WAY THE WINNING TREES DO: *** "
   "sp1-lean has ZERO numeric-literal offsets across 396 files (`env.get (<literal>)`: 0 occurrences; "
   "`env.get (i0 + ...)`: 1540) — offsets stay symbolic so they match `circuit_norm` output syntactically. "
   "Clean has ZERO `maxHeartbeats` in its entire tree and its CLAUDE.md says 'Never modify maxHeartbeats'; "
   "sp1-lean's docs call a maxHeartbeats bump in a simple proof 'a code smell, not a fix'. *** SPLITTING A "
   "BIG DECLARATION REALLY DOES BUY BUDGET — BUT NOT THE WAY YOU MIGHT THINK, AND THE GOTCHAS ARE NOW READ "
   "OFF THE v4.28.0 SOURCE, NOT GUESSED. *** `maxHeartbeats` is documented PER COMMAND (`CoreM.lean`: "
   "'maximum amount of heartbeats per command', default 200000), and the check is `numHeartbeats - "
   "initHeartbeats > max` with `initHeartbeats` snapshotted per lift of CoreM — so N separate `theorem` "
   "commands genuinely get N separate budgets. THAT is the win. *** WHAT DOES NOT BUY BUDGET: `Elab.async`. "
   "*** `Core.wrapAsync` DELIBERATELY PRE-CHARGES each forked task with what the parent already spent "
   "(`IO.addHeartbeats heartbeats`, with the in-source comment 'such that forking off an action doesn't "
   "suddenly allow it to succeed from a lower heartbeat count'). Toggling async will NOT make a timing-out "
   "proof pass. *** FOUR SILENT WAYS TO LOSE ASYNC ELABORATION (`Elab/MutualDef.lean`, the literal guard is "
   "`if let (#[view], #[declId]) := ... && view.kind.isTheorem && ... && !headers[0]!.type.hasMVar`): (a) "
   "MORE THAN ONE DECLARATION — every `mutual` block is fully sequential; (b) NOT A `theorem` — `def`, "
   "`instance`, `abbrev` and `example` never get async bodies, so DO NOT BENCHMARK WITH `example`; (c) a "
   "METAVARIABLE IN THE THEOREM TYPE — `_` holes, AND `hasMVar` covers UNIVERSE mvars too; (d) "
   "`deprecated.oldSectionVars`. Case (c) can be COMPLETELY SILENT: the explanatory message is `discard`ed "
   "and is further suppressed if any other error was already logged in that command. DETECT IT WITH `set_option "
   "trace.Elab.async true` AND LOOK FOR THE ABSENCE OF AN `elaborating proof of <name>` NODE — that is the "
   "only reliable signal. *** FIXING THIS "
   "ONE MODULE UNBLOCKS EVERY QUEUED SHA-256 TECHNIQUE AT ONCE. ***"),
  ...
  ("survival first — BUT WE HAVE NOW BANKED ONE, AND IT SETTLES THE WALL QUESTION","*** THE FULL SUBMISSION LEDGER ON THIS SLUG, AND IT SETTLES THE WALL QUESTION AGAINST THE OBVIOUS READING. -612 TIMED OUT. -306 TIMED OUT. -298 VERIFIED AND TOOK THE RECORD (144,687, from yash1io's 144,985). -280 TIMED OUT TWICE. TWO FACTS, AND THE SECOND IS THE ACTIONABLE ONE. (1) SIZE DOES NOT PREDICT THE OUTCOME: the SMALLEST reduction we have attempted here timed out while a LARGER one verified, so AIMING SMALLER IS NOT A STRATEGY and has now cost four attempts. (2) EVERY SINGLE FAILURE ON THIS SLUG IS A VERIFIER TIMEOUT — not one soundness failure, not one bad ledger. So the binding constraint is ELABORATION TIME AND NOTHING ELSE, and the -280 at 144,389 IS A SOUND SOLUTION THAT SIMPLY WILL NOT ELABORATE IN BUDGET. *** RETRY AN IDENTICAL SOLUTION RATHER THAN ABANDONING IT — BUT READ WHICH FAILURE YOU GOT. *** The harness reports two DIFFERENT terminal failures and they mean opposite things. `VERIFIER TIMEOUT` means zk.golf returned status=timeout: the verifier gave up, your solution was never judged, and RESUBMITTING THE SAME TREE IS THE CORRECT MOVE. `failed score=None` means status=failed or error: zk.golf judged it and rejected it — a broken build, a bad axiom, or an unsound circuit — and resubmitting unchanged will fail again. MEASURED LEDGER ON THIS SLUG, and the timeouts really are stochastic: the same -280 landing at 144,389 was submitted FIVE times — one VERIFIED AND TOOK THE RECORD, three timed out, one hard-failed. Two of the timeouts arrived AFTER the successful run on the identical tree, so a timeout is evidence about that RUN and nothing else. Earlier attempts: -612 timed out, -306 timed out, -298 verified first try. *** A TIMEOUT IS NOT EVIDENCE THAT YOUR SOLUTION IS SOUND — IT IS EVIDENCE OF NOTHING AT ALL. *** Measured both ways on this slug: 144,389 timed out repeatedly and then VERIFIED on retry, while 144,353 timed out once and then came back HARD-REJECTED (`failed score=None`) when a retry actually got judged. Same signal, opposite truths underneath. So retrying is how you FIND OUT which case you are in, not a way of winning. *** THE -36 FROM THE 144,389 SEED IS A DEAD END — FOUR ATTEMPTS, THREE DETERMINISTIC REJECTIONS. *** It landed at 144,353 and was submitted four times: one VERIFIER TIMEOUT and then THREE `failed score=None`. *** READ THAT AS A CHANGE, NOT AS A SCORE. The seed has since moved to 144,373, so 144,353 is now reachable by a DIFFERENT -20 that has nothing to do with the defective one, and a job hitting that total today is not necessarily repeating the mistake. What is condemned is the specific reduction that the 144,389 seed made obvious, not the number 144,353. Identify your change and judge it on its own; if it is genuinely a different edit, submit it. *** A rejection is not a timeout — the verifier JUDGED it and threw it out, so it is deterministic and three of them at the same score is not variance, it is a genuine defect in that reduction (a broken build, a disallowed axiom, or an unsound row). CONTRAST IT WITH 144,389, WHICH LOOKED SIMILAR AND WAS NOT: that one was three TIMEOUTS plus one rejection, and it verified on the fifth try. Timeouts are worth retrying; rejections are not. If you rediscover the -36 at 144,353 — and four jobs before you did, so you probably will — DO NOT SUBMIT IT UNCHANGED. Either find the defect first, or take a different reduction. WHAT TO DO WITH YOUR SLOT. (1) On a TIMEOUT, resubmit unchanged once or twice — it is cheap and it converts an unknown into an answer. Do not trade the reduction down for a safer one you merely fear will time out. (2) On a `failed score=None`, STOP RESUBMITTING and find the defect: the verifier judged it and rejected it, so the build, the axioms or the soundness proof is genuinely wrong and an identical resubmission cannot pass. (3) Buy headroom in the SAME submission either way, since it raises the per-run success probability and a cost-neutral tidy-up cannot be submitted alone — BUT NOT WITH `decide +kernel`, WHICH IS THE WRONG TOOL HERE AND MAY BE PART OF THE PROBLEM. Measured on this tree: the import closure of Solution.SHA256.Main is 156 files containing 5,073 `norm_num`, 3,253 `omega`, 2,063 `simp only`, 1,507 `rfl` and only 30 `decide` — and ZERO `decide +kernel`. There is nothing here for it to convert. Worse, moving work kernel-side trades a clean fast failure for a slow ugly one — but READ THE MECHANISM CORRECTLY, because an earlier version of this note had it wrong and you will misread your own log if you inherit that error. It is NOT true that the kernel escapes `maxHeartbeats` entirely: since Lean 4.9.0 (PR #4113) `Kernel.Environment.addDecl` passes `Core.getMaxHeartbeats opts` into `addDeclCore`, and `environment.cpp` arms it with `scope_max_heartbeat`. THE CATCH IS THAT THE TWO LIMITS ARE THE SAME NUMBER IN DIFFERENT CURRENCIES: the elaborator's counter is a thread-local ALLOCATION count (`runtime/alloc.cpp`), while the kernel's is a separate counter incremented once per `check_system` CALL (`runtime/interrupt.cpp`) — vastly rarer, so the kernel's effective budget is enormously larger. IN PRACTICE THE KERNEL DIES ON STACK, NOT ON HEARTBEATS: `check_system` calls `check_stack` unconditionally but heartbeats only conditionally, so what you will actually see is `(kernel) deep recursion detected` or a bare `Stack overflow detected. Aborting.` — NOT a `(deterministic) timeout`. Measured on v4.28.0: `Nat.rec (motive := fun _ => Nat) 0 (fun _ ih => ih) 10000 = 0 := rfl` passes the ELABORATOR and then dies in the KERNEL with `deep recursion detected`. So if you see `deep recursion` or a stack abort with no timeout, that is kernel work, and raising `maxHeartbeats` cannot help you. NOTE ALSO that under `Elab.async` the kernel check runs in a BACKGROUND TASK, so that error surfaces DETACHED from the tactic block that caused it — do not attribute it to whatever declaration is printed nearby. AND `@[irreducible]`/`seal` DO NOT PROTECT THE KERNEL — the kernel has no access to reducibility annotations at all (`type_checker.cpp` says so in a comment); only `opaque`/Mathlib's `irreducible_def`, which emit a constant with NO BODY, are invisible to it. WHAT TO DO INSTEAD, IN ORDER: (i) the 5,073 `norm_num` calls are the biggest single count and most of them discharge closed literal index bounds like `0 < 8` — `norm_num` is `simp` PLUS extension dispatch over the full Mathlib simp set and is never cheaper than `simp` on the same goal, so step down the ladder `norm_num` -> `norm_num only [...]` -> `norm_num1` (which does not call simp at all), checking each site compiles rather than sweeping blind — AND EXCLUDE PRIMALITY GOALS FROM THAT PASS: `Nat.minFacAux` is well-founded recursive and therefore irreducible since Lean 4.9.0, so `decide` cannot reduce it and Mathlib documents `norm_num` as MUCH FASTER there, the opposite of the general rule; (ii) `omega` costs scale with CONTEXT size, not goal size, because it processes every hypothesis — hoisting a repeated shape into one lemma proved once and applying it with a closed side condition measured 2.8x faster, and `omega -splitNatSub -splitMinMax` removes default-on search that most sites do not need; (iii) `irreducible` on heavy definitions still stands. (4) DO NOT AIM SMALLER TO DODGE THE WALL — the smallest reduction attempted here (-280) needed five tries while a larger one (-298) verified immediately. Size does not predict the outcome. BUY ELABORATION HEADROOM BEFORE YOU REDUCE ANY "
   ...
   "definitions irreducible. *** DO NOT BLANKET-CONVERT `by decide` TO `decide +kernel` — that advice was derived from the fixed-base tree (1,659 `decide` sites) and does NOT transfer here, and it can make timeouts WORSE — though not for the reason an earlier version of this note gave. The kernel IS heartbeat-bounded since Lean 4.9.0, but its counter ticks once per `check_system` call rather than per allocation, so the effective budget is far larger and the kernel characteristically dies on STACK instead: expect `(kernel) deep recursion detected` or a bare stack abort, NOT a `(deterministic) timeout`. Either way the wall clock is spent. Note too that `@[irreducible]` and `seal` are invisible to the kernel — it ignores reducibility annotations entirely — so they buy elaborator speed only; only `opaque`/`irreducible_def` (which emit a body-less constant) stop the kernel from unfolding. `decide +kernel` is correct in exactly one narrow case — a bounded-quantifier decide over a LARGE range, where it is about 1.5x faster and, measured at N=4000, survives where plain `decide` aborts with a stack overflow. On small closed goals it measured about 25% WORSE, because it creates an auxiliary declaration for the kernel. *** Targets for irreducible: cost constants, the big params and schedule tables, any def whose body is a large "
