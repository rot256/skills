...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (47436, 49536, [
 ...
 "secp256k1-scalar-mul": (304662, None, [
  ...
  ("19800 if it is real, and one grep settles that","AUDIT THE LOOP CERTIFICATES FOR ONES STILL ON THE 4x64 FOLD. "
   "ARITHMETIC THAT MOTIVATES THIS AND NOTHING MORE: folds are 31.8% of glvStep = 1,394 per step, spread over about "
   "5 certificates, i.e. AN AVERAGE OF 278 EACH. But the measured 32-bit certificate CERT32 is <96,98> = 194 and it "
   "reconciles exactly against its parts (30 + 71 + 93), while the 64-bit CERT64 is 354. An average of 278 between "
   "194 and 354 implies ROUGHLY TWO CERTIFICATES PER STEP ARE STILL AT 64. At -160 each that is -320 per step x 62 "
   "= -19,840. THIS IS AN INFERENCE FROM AN AVERAGE, NOT A SIGHTING — the first thing to do is enumerate every fold "
   "instance on the live step path and record its actual cost constant, and if they are all already 32-bit then SAY "
   "SO AND STOP, because a pattern that has paid before is a reason to CHECK the next instance, never a reason to "
   "assume it. If two are genuinely at 64, porting them is the same mechanical change that already landed "
   "elsewhere in this tree (DivOrZeroN32 -> Fold32N), so the fix is low-risk once the sighting is confirmed."),
