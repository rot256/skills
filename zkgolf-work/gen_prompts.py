...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (49218, 49536, [
  ...
  ("372 — SURGICAL, NO PARAMETER FILE CHANGES","REPLACE interpolatedMul + foldLhs WITH A CRT MULTIPLY IN THE RESIDUE "
   "RING. interpolatedMul pins the full 2m-1 = 7-coefficient convolution, but NOTHING EVER CONSUMES THOSE SEVEN "
   "COEFFICIENTS — every consumer reads foldLhs, i.e. z mod (X^4 - cF). Multiplication in F_r[X]/(X^4 - cF) has "
   "bilinear rank 4, NOT 7, because cF = 2^32 + 977 IS A FOURTH POWER MOD THE BN254 SCALAR FIELD (r - 1 has 2-adicity "
   "28, so mu_4 lies in F_r) and X^4 - cF therefore SPLITS COMPLETELY. THE FOUR ROOTS, VERIFIED: "
   ...
   "15865747286151745787645593793184946820902319848935289103006992069981740477241. THE GADGET: witness F : Vector _ "
   "4; for each root rho_j emit ONE row (SUM a_i rho_j^i) * (SUM b_i rho_j^i) = SUM_k F_k rho_j^k — one product, all "
   "coefficients compile-time constants. foldLhs then becomes F[k] + wExp v k, still affine, and nfFoldL / nfFoldWL / "
   "offFold* / wfFold* are UNCHANGED. <7,7> = 14 -> <4,4> = 8 per product: stageC1 -12, stageX -6, i.e. -18 per stage "
   "x 20 = -360, plus stageAF -6 and ChainFinishFold -6. FILES: new crtFoldMul beside MulMod.interpolatedMul in "
   "InterpMul.lean; FoldQuot.lean (foldLhs, foldLhs_lt); FoldWide.lean (foldLhs2); call sites ChainFold.lean:39,169,302, "
   "PairAddFoldA/C1, ChainFinishY/Fold. LEAN: soundness is Vandermonde.interp_uniqueness at N = 4 with f = rho_j "
 ...
 ("928 — A PURE DUPLICATION THE CIRCUIT IS PAYING FOR TWICE","DELETE BOTH Canonicalize.main CALLS AND REUSE THE "
  ...
  ("2060 to 5285 — RESOLVE THE MIXED-RADIX QUESTION FIRST","MOVE THE WITNESSED LIMBS TO BASE 2^32. Materialising a "
   "256-bit value costs EXACTLY 512 at any limb width (NormalizeImplicit is <m*(B-1), m*B>, so 4+508 = 8+504), so "
   "limb width is FREE TO CHOOSE and what it changes is the MAGNITUDE BUDGET that sets the quotient and carry widths "
   "— and those are LINEAR in limb width at 2 score per bit. ANY CLAIM IN THE CORPUS THAT 32-BIT LIMBS ARE A NET LOSS "
   "IS RETRACTED: it compared only normalisation (a wash) and product count (+48/site) and never recomputed W_q or "
   "W_c. TWO VERSIONS, AND YOU MUST DECIDE BETWEEN THEM BEFORE EDITING. PARTIAL, -2,060, ZERO SELECT COST: witness "
   "lambda and x as BigInt 8 at B=32 and define the 4x64 view by the FREE AFFINE RECOMBINATION lam64[i] = lam32[2i] + "
   "2^32*lam32[2i+1], leaving every table and select untouched. Per stageX site: materialisation 512 -> 512 (a wash), "
   "lambda^2 <7,7> -> <15,15>, quotient 67 bits (134) -> 38 bits (75), grouped carry W=98 (196) -> W=70 (140), stage "
   "856 -> 758, across ~21 sites. This CANNOT shrink stageC1, whose multiplicand IS a selected table limb. FULL, "
   "-5,285: also convert stageC1 and use the fold modulus X^8 - X - 977, which at base 2^32 is LIMB-ALIGNED (2^256 = "
   "2^32 + 977 means X^8 = X + 977 at X = 2^32) and is the UNIQUE degree-8 modulus vanishing at 2^32 mod p with all "
   "coefficients below 2^31. It factors (1)(2)(2)(3) — one linear root "
   "4057929584660596922171240314385854169924567973749597780255708884483859498814, two quadratics, one cubic — so the "
   "residue-ring rank is 1+3+3+5 = 12, <12,12> = 24 per product, and the 8 folded coefficients are an AFFINE "
   "(constant CRT matrix) function of the 12 witnesses. Because the growth factor drops from cF ~ 2^32 to 977 ~ 2^10 "
   "AND the unreduced low half drops from 4*2^320 to 8*2^288: narrow x' quotient 36 bits (72) + 24 products + carry "
   "g=6 W=43 (86) = CERT 182 against 344; wide lambda' quotient 38 bits (76) + 48 products + carry g=5 W=46 (92) = "
   "CERT 216 against 366; i.e. -312 per stage. All GVXHyps side conditions check numerically (group-0 native row "
   "2^237.6 < r, final row 2^100 < r, 16 bits of slack), twoPBorrowDigit in base 2^32 is feasible (the eight digits "
   "all lie in [2^32, 2^33)), and the c976 trick for x_d is unchanged. THE COST OF THE FULL VERSION is that Select12 "
   "must emit x on 8 limbs, xProd 60 -> 120, +60 per select x 21 = +1,260; y STAYS 4x64 since it is only an additive "
   "RHS term. DO NOT USE X^8 - cF: it splits completely and has rank 8 rather than 12, but its folded coefficients "
   "reach 2^99 forcing a 66-bit carry, giving certs 220/244 against 182/216 — THE 22-BIT-SMALLER COEFFICIENTS BEAT "
   "THE FOUR EXTRA PRODUCTS. FIRST THING TO CHECK: whether stageC1 can instead use a MIXED-RADIX product, 8 "
   "coefficients at 2^32 against 4 at 2^64, which would capture the wide-site saving WITHOUT the +1,260 "
   "retabulation. If mixed radix works the combined figure is about -6,500, better than either version."),
  ("11115 — THE BIG ONE, ATTEMPT AFTER THE BASE-2^32 WORK LANDS","THE SQUARES TELESCOPE: STOP MATERIALISING THE "
   "ACCUMULATOR x ENTIRELY. Unroll x_j = lambda_j^2 - x_{j-1} - xT_j and every intermediate x-coordinate is an "
   "ALTERNATING SUM OF SQUARES: x_j = sum_{i<=j} (-1)^(j-i) lambda_i^2 + (-1)^j x_0 + sum_i (+-xT_i). interpolatedMul "
   "returns its convolution positions as AFFINE WIRES and affine recombination is free, so EVERY x_j IS AVAILABLE FOR "
   "FREE AS AN UNREDUCED CONVOLUTION once the lambda_i^2 exist. The state becomes THE LAMBDAS ALONE. Per window at "
   "B=32: witness lambda_j as BigInt 8, range-checked 8x32 = 256 bits, <8,0> + <248,256> = 512; lambda_j^2 by "
   "interpolatedMul m=8 = <15,15> = 30, 15 positions each < 8*2^64 = 2^67; x_j := the telescope, FREE, 15 positions < "
   "21*8*2^64 = 2^71.4, pre-folded onto 8 positions with cF (free) at < 2^104.4; then ONE reduction site, the slope "
   "identity in T-relative implied-y form using the tree's existing Chain.impliedY trick: lambda_j*(xT_j - x_{j-1}) + "
