...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (47435, 34030, [
  ...
  ("0 — THE -1 AT 47,413 IS SETTLED NEGATIVE, 0-FOR-3, AND tick.py NOW BLOCKS IT","*** DO NOT READ THIS AS "
   ...
   "THE SUBSTITUTED EXPRESSION, which excludes no environments and is why the verifier threw them out. *** 0-FOR-5 NOW, AND THE DIFFERENCE BETWEEN THE TWO SITES IS "
   "FINALLY LOCALISED — READ THIS INSTEAD OF ATTEMPTING A SIXTH CERTIFICATE. *** The fifth attempt was the "
   "best-shaped one: it added NO new file and instead extended FoldLinIdent.lean and FoldQInv.lean, the machinery "
   "that already verifies at stageX_scw. Rejected too. *** TWO THEORIES OF MINE ARE DEAD, SO DO NOT REPEAT THEM. "
   "*** (a) It is NOT a missing quotient range check: ChainFold.lean:40-41 shows stageO2 ALREADY has `assertion "
   "(RangeCheck.circuit 67 ...) (CrtMul.crtQInv convLL (4 * P256) x1 xA xT1)`, identical to stageX's at :307-308. "
   "(b) It is NOT the shape of the fold: both stages witness an x, call NormalizeImplicit, call `crtFoldMul lam "
   "lam`, range-check crtQInv at width 67, and fold the same crtLhs against foldRhs over the same `64 gfFold "
   "posOfFold 3 vFoldL vFoldR hgvFold`. THE FOLD OBLIGATION IS THE SAME SHAPE AT BOTH SITES. *** WHAT ACTUALLY "
   "DIFFERS IS ONE THING. *** stageX takes `xPrev xT lam2` and EVERY INPUT APPEARS IN THE FOLD. stageO2 takes `xA "
   "yA xT1 yT1 lam1` and *** yA AND yT1 DO NOT APPEAR IN THE FOLD AT ALL. *** stageX's lam2 is a DOUBLING slope; "
   "stageO2's lam1 is an ADDITION slope whose relation to the y-coordinates, lam1 = (yT1 - yA)/(xT1 - xA), lives "
   "OUTSIDE this fold. *** SO ANSWER THIS BEFORE WRITING ANY LEAN: with the top row deleted, DOES ANYTHING STILL "
   "PIN lam1 AGAINST yA AND yT1? *** It is a question about the SITE, answerable by reading the caller, and it "
   "needs no certificate. If the deleted row is what closes that loop, the site is genuinely closed, five "
   "rejections is the correct answer rather than bad luck, and the slot belongs elsewhere. If something else closes "
   "it, say what — and then copy the competitor's argument shape, which is that the CARRY LOOP already proves "
   "equality over the bounded integer representatives, never that the row is redundant given the affine quotient."),
