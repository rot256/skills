...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (47435, 34030, [
  ("1300 to 1700 — THE SELECTOR IS NOT AT ITS FLOOR AND THE BETTER CONSTRUCTION IS VERIFIED OVER BN254. TAKE "
   "THIS FIRST: IT IS SMALL, LOCAL, AND NEEDS NO NEW SOUNDNESS ARGUMENT","*** TWO ENTRIES IN THIS CORPUS SAY "
   "SELECTION IS 'A SOLVED PROBLEM' AND 'PROVABLY OPTIMAL'. BOTH ARE WRONG, AND THE FIX IS PURE LINEAR ALGEBRA "
   "— NO SOLVER, NO SEARCH, NO POLYNOMIAL SYSTEM. *** They are right that Select12 attains the PEEL optimum "
   "R(w,m) = min_j [m(2^j - 1) + 2^(w-j) - (w-j) - 1] ~ 2*sqrt(m*2^w), which is also the published state of "
   "the art (Boyar and Find, TCS 2018, Thm 7, arXiv:1407.6169). They are wrong that peel is a FLOOR. *** THE "
   "IDENTITY, WHICH IS THE WHOLE THING. *** Split the 11 window bits as t + j and BUILD BOTH ONE-HOT VECTORS, "
   "cost (2^t - t - 1) + (2^j - j - 1). Every affine form over the witness is then ADDITIVE, alpha_a + "
   "gamma_c — a function of the high index plus a function of the low index. So for any two of them: "
   "(alpha_a + gamma_c)(beta_a + delta_c) = alpha_a*beta_a + gamma_c*delta_c + (alpha (x) delta + beta (x) "
   "gamma). *** THE FIRST TERM DEPENDS ON a ALONE, SO IT IS A FREE AFFINE COMBINATION OF THE A ONE-HOT YOU "
   "ALREADY BUILT; THE SECOND DEPENDS ON c ALONE, SO IT IS FREE OVER THE C ONE-HOT. THE ROW THEREFORE "
   "DELIVERS THE RANK-2 MATRIX FOR NOTHING. *** Each target, viewed as a 2^t x 2^j matrix, costs "
   "ceil(rank/2) rows instead of peel's 2^j - 1. That is a factor of sqrt(2) asymptotically. *** THE NUMBER, "
   "AND ONE DISCREPANCY YOU MUST RESOLVE BEFORE BUDGETING. *** Closed form at t=4, j=7 is 11 + 120 + 8m = 179 "
   "at m=6. A DP that folds this together with peel reports 170 at m=6 (the two AGREE at m=7, both 187). WE "
   "RUN m=6 TODAY, and the current Select12 is 210 rows. SO: -31 rows per select at the conservative 179, or "
   "-40 at 170; times 21 selects that is 651 or 840 rows, i.e. *** -1,302 TO -1,680 SCORE. *** BUDGET THE "
   "CONSERVATIVE FIGURE AND RE-DERIVE THE m=6 OPTIMUM BY HAND — the construction is small enough to check "
   "directly, and do not ship a cost ledger you have not re-derived. *** VERIFICATION ALREADY DONE, DO NOT "
   "REDO IT: *** every row was checked over the ACTUAL BN254 SCALAR PRIME (not a toy prime) — each product's "
   "two factors verified to be affine forms over the current witness, each output verified to be an affine "
   "form, against random target tables. Full table (peel / new): m=1 88/69, m=2 119/99, m=3 150/118, m=4 "
   "180/136, m=5 195/153, m=6 210/170, m=7 225/187. *** WHY THIS IS THE EASIEST BIG ITEM ON THE BOARD. *** "
   "The edit is confined to the Comb/Select family (Comb, CombCW, CombComplete, CombElaborated, CombSound, "
   "SelectCost, SelectStable, SelectTheorems, Main), exactly as the m: 7 -> 6 reduction was — that one landed "
   "for its predicted -630 to the unit. It does NOT touch ll_effNT, so it is independent of the stageO2 "
   "history. AND THE SOUNDNESS OBLIGATION IS THE SAME SHAPE AS TODAY'S: the one-hot cells are still pinned by "
   "the same polynomial identities, there is no new exceptional case, and no denominator or canonicity "
   "assumption changes. *** DO NOT CHASE THE REST OF THE GAP. *** The counting bound is ~106 and a shared-"
 ...
 "secp256k1-scalar-mul": (304224, None, [
  ("~40000 — THE QUESTION IS ANSWERED AND THE ANSWER IS YES; THIS IS NOW A RECIPE, NOT A SEARCH","*** DELETE THE "
   "ACCUMULATOR y FROM THE FUSED STEP. FOUR MATERIALISATIONS SUFFICE, THE ALGEBRA IS VERIFIED, THE SOUNDNESS IS "
   "VERIFIED EXHAUSTIVELY, AND IT TOUCHES NO GUARD. *** Do not go looking for rows to shave inside the step — "
   "there are none at five materialisations; a materialised 256-bit F_q element costs 512 (2 per bit) and a "
   "degree-2 certificate 196. The lever is dropping a materialisation, and here is exactly how. *** WHAT ELM DOES "
   "NOT DO. *** fusedStep is Eisentraeger-Lauter-Montgomery verbatim (arXiv:math/0208038 section 3.1) and ELM "
   "eliminates ONE y — the y of the intermediate S = A+T. It does NOT eliminate the y of the ACCUMULATOR, because "
   "ELM is a single-operation paper with no chain recursion. But y_i occurs EXACTLY THREE TIMES in the whole "
   "chain and is LINEAR WITH UNIT COEFFICIENT in all three: its definition y_i = A_i(x_i - x_{i-1}) - y_{i-1} in "
   "FinishXY, its use in SlopeXS (lambda_{i+1}(x_T,i+1 - x_i) = y_T,i+1 - y_i), and its use in Slope2 "
   "(w_{i+1}(x_S,i+1 - x_i) = 2 y_i). *** SOLVE THE SlopeXS USE FOR y_i AND SUBSTITUTE. *** That is a "
   "SUBSTITUTION, not a relaxation — solution sets are in bijection, so there is no soundness loss to argue "
   "about. Three relations and one wire become two relations and no wire: E1_i: w_i(x_S,i - x_{i-1}) + 2 lambda_i "
   "(x_T,i - x_{i-1}) = 2 y_T,i, and E2_i: lambda_{i+1}(x_T,i+1 - x_i) - (lambda_i + w_i)(x_{i-1} - x_i) + "
   "lambda_i(x_T,i - x_{i-1}) = y_T,i+1 + y_T,i. BOTH DEGREE 2; x_T and y_T are free affine table reads. This is "
   "the fixed-base R1 recursion ported to the FUSED step, which nobody had done because ELM's y-elimination and "
   "the chain's y-elimination look like the same trick and are not. *** THE STEP: *** materialise lambda_i, "
   "x_S,i, w_i, x_i; certificates C1: lambda_i^2 = x_S,i + x_{i-1} + x_T,i (1 product), C2: (lambda_i + w_i)^2 = "
   "x_i + x_S,i + x_{i-1} (1 product), plus E1 (2 products) and E2 (3 products). Four witnesses, four relations. "
   "Boundary: lambda_1(x_T,1 - x_0) = y_T,1 - y_0 once at the head where y_0 is known, and y_L materialised once "
   "at the tail. EMIT E2_i AT THE HEAD OF STEP i+1, where x_{i-1}, x_i, lambda_i, w_i and both table reads are "
   "all available as free affine inputs. *** VERIFICATION, ALL ON REAL secp256k1: *** ELM step reproduces 2A+T "
   "over 1,600 steps with 0 violations; E1 and E2 on real chains, 1,600 steps, 0 violations; the full 4-mat "
   "system with NO y wire anywhere, 40 chains x 62 steps, 0 failures. AND AN EXHAUSTIVE SOUNDNESS CHECK: over "
   "y^2 = x^3 + 7 on F_43 with L=2, brute force over ALL (lambda_1, w_1, lambda_2, w_2) in F_43^4 — 21,870 "
   "chains, EXACTLY ONE SOLUTION EACH, ALWAYS THE HONEST ONE, zero spurious and zero missing. Uniqueness is "
   "STRUCTURAL: the reduced system is triangular and linear in the solve order, so the prover has zero freedom. "
   "*** IT DEGENERATES EXACTLY WHERE THE TREE ALREADY GUARDS: *** x_T,i = x_{i-1} (T = +-A) or x_S,i = x_{i-1} (S "
   "= -A), i.e. PRECISELY THE sameX1/sameX2/cancel/zOut FLAGS SlopeXS AND Slope2 ALREADY CARRY. The elimination "
   "opens NO new exceptional case. *** COST, FROM THIS TREE'S OWN PROVED CONSTANTS: *** deleted — y4's whole "
   "subcircuit (mulModSub2W32M <422,425> = 847), the m2y Emu mux (~8), AND BOTH DIVISION FOLDS (2 x 196), because "
   "E1 and E2 now do the pinning divOrZeroF3 and divOrZeroN32 did — 1,247 removed. Added — E1 (2 products) and "
   "E2 (3 products), ASSERTION-ONLY, at 280+400 conservative or 230+300 optimistic. NET -567 TO -717 PER STEP, "
   "-35,154 to -44,454 over 62 steps, projecting 267,900 down to 258,600. *** IT WORKS AT 4x64 AS-IS — every "
   "certificate stays degree 2 and nothing needs 8x32 (D2 overflow reaches 2^149.8, +104 bits of margin). *** AND "
   "THE x-RECURSION SELF-TERMINATES, WHICH IS BETTER THAN THE FIXED-BASE ANALOGUE: substituting x_S = lambda^2 - "
   "x_prev - x_T into x_4 = (lambda+w)^2 - x_S - x_prev gives x_i = w_i(w_i + 2 lambda_i) + x_T,i WITH x_{i-1} "
   "CANCELLING IDENTICALLY (3,000 real steps, 0 violations). No 21-deep alternating sum, no coefficient growth, "
   "no 8x32 requirement. *** THEN THREE AND TWO, BUT ONLY AFTER 4-MAT LANDS. *** 3-mat (lambda, w, x): drop x_S, "
   "C1 vanishes, C2 becomes w(w + 2 lambda) = x - x_T, E1 rises to degree 3 — about -1,150/step, projecting "
   "~231,600. 2-mat (lambda, w): also drop x, E1 and E2 both degree 3 — about -1,720/step, projecting ~196,100. "
   "Both want 8x32 (D3 at 4x64 leaves only 26-29 bits of margin). BOTH NEED GUARD REDESIGN — 3-mat "
   "un-materialises x_S which feeds sameX2, 2-mat un-materialises x which feeds sameX1 and the DivOrZeroF3 "
   "borrow-free side condition. 4-MAT TOUCHES NO GUARD, NO DENOMINATOR AND NO CANONICITY ASSUMPTION: TAKE IT "
   "FIRST. *** AND THE FLOOR IS TWO, NOT ONE — DO NOT CHASE ONE. *** Materialisations = number of independent "
   "DIVISIONS. x_{2A+T} has cleared denominator d1^2 D2^2 with d1 = x_T - x_A and D2 = num1^2 - (2x_A + x_T)d1^2, "
   "and these are COPRIME, so two distinct irreducible poles need two witnesses; collapsing to one witness u = "
   "1/(d1 D2) makes x_new degree 8, far past the D<=3 ceiling. ELM's own Table 1 says 2P+-Q costs 2 squarings, 1 "
   "multiplication, TWO DIVISIONS — and divisions are exactly what cannot be expressed polynomially and must be "
   "witnessed. *** SIX DIRECTIONS ARE "
 ...
 "sha256-hash": (144985, None, [
  ("READ THIS FIRST. YOUR TREE WILL DIE AT FOUR NAMED LINES UNLESS YOU FIX THEM, AND THE LAST THREE TREES "
   "ALL DIED AT EXACTLY THOSE LINES","*** THE VERIFIER LOG IS NOW READ AUTOMATICALLY ON EVERY FAILURE, AND IT "
   "SAYS THE SAME THING EVERY TIME. *** Three consecutive sha256 submissions — dd8e5c0e, d9199778 and their "
   "predecessors — were BYTE-DIFFERENT TREES that produced the IDENTICAL FOUR ERRORS:\n"
   "  ComputableDCompressorsFull.lean:498:4  (deterministic) timeout at `whnf`, heartbeats (2000000)\n"
   "  ComputableDCompressorsFull.lean:528:2  (deterministic) timeout at `tactic execution`, heartbeats (2000000)\n"
   "  ComputableDCompressorsFull.lean:568:0  (deterministic) timeout at `whnf`, heartbeats (200000)\n"
   "  ComputableDCompressorsFull.lean:608:10 (deterministic) timeout at `whnf`, heartbeats (200000)\n"
   "*** THAT IS A PERFECTLY REPRODUCIBLE FAILURE AT FOUR FIXED SITES, AND REVISING THE CIRCUIT DOES NOT TOUCH "
   "IT. *** Jobs have now spent three submissions changing the arithmetic and re-hitting the same four lines. "
   "If you ship a tree that reaches ComputableDCompressorsFull unchanged, YOU WILL GET THIS EXACT FAILURE AND "
   "WASTE THE QUEUE SLOT. *** WHAT IS AND IS NOT THE PROBLEM. *** It is NOT soundness — the verifier never "
   "reaches the soundness question, so none of the techniques that died here (W4, the six-lane deferral) has "
   "actually been refuted; they are UNAFFORDABLE, not wrong. It is NOT the row count. It is ELABORATION COST "
   "in four declarations of one machine-generated module. *** AND RAISING THE CEILING IS PROVED USELESS THREE "
   "TIMES OVER: *** these trees already ran at 200,000, then 2,000,000, then 4,000,000, and every one blew. "
   "zk.golf's own llms.txt warns that raising maxHeartbeats can push the build past the HARD 1200-SECOND WALL "
   "CLOCK, converting a clean heartbeat error into a worse one. DO NOT TOUCH maxHeartbeats. *** DO THIS "
   "INSTEAD, IN THIS ORDER. *** (1) RUN `set_option diagnostics true` ON THAT FILE FIRST — nobody ever has, "
   "and its 'unfolded declarations' list names the offending subcircuit directly, along with 'heuristic for "
   "solving f a =?= f b' counts that reveal unification thrash. (2) SPLIT the four flagged declarations into "
   "many small `theorem`s: maxHeartbeats is PER COMMAND, so N declarations get N budgets, and Elab.async "
   "parallelises PER DECLARATION so one giant proof gets one core. (3) Check the theorem STATEMENTS for `_` "
   "holes — a metavariable in a type both costs elaboration and SILENTLY disables the parallelism, with no "
   "diagnostic. (4) Wrap the expensive subcircuit in `irreducible_def` so isDefEq fails fast instead of "
   "unfolding it, and unfold deliberately via the generated `_def` lemma. *** FIXING THIS ONE MODULE UNBLOCKS "
   "EVERY QUEUED SHA-256 TECHNIQUE AT ONCE, which is why it is worth more than any of them individually. ***"),
  ...
  ("0 — THE W4 / Numeric4W ROUTE IS DEAD AND IT WAS NEVER NECESSARY. READ WHAT ACTUALLY WON.","*** FOUR OF "
   "OUR TREES REACHED 144,321 BY WEAKENING A DEFERRED-STATE BOUND (RPShared.Numeric5W -> Numeric4W on state words "
   "3 and 7, with a W4 round family and a truncated 31-lane upper-Sigma0). ALL FOUR WERE JUDGED AND REJECTED. *** "
   "Then a competitor went STRAIGHT PAST that score to 144,315 WITHOUT THE TECHNIQUE AT ALL: their tree — which "
   "is now your seed — has NO Numeric4W.lean, ZERO occurrences of Numeric4W, ZERO of the W4 round family, and "
   "Numeric5W still in eleven files. *** SO DO NOT SPEND A FIFTH SLOT ON THAT BOUND. It was a detour, and whether "
   "it is even sound is now moot. *** WHAT WON INSTEAD IS A DIFFERENT KIND OF IDEA — AN INPUT-STRUCTURE FACT, NOT "
   "AN ARITHMETIC BOUND. *** The new files carry the name W15Low3, and Block3W15Low3.lean states it plainly: 'The "
   "final byte of witnessed block 3 is byte 255. SHA-256's bit length is ALWAYS A MULTIPLE OF EIGHT, so its three "
   "least-significant bits are ZERO.' They then OMIT those witness bits. Supporting work: "
   "CompressBlockWideSparseDPackW15Low3, ComputableDCompressorW15Low3, CostW15Low3R1CS, a block-4 bundle "
   "(CompressBlock4, Block4CircuitBundle, Block4ProofBundle, Cost4Base, Cost4Schedule, "
   "ComputableMessageSchedule4) and ComputableScheduleStepUnchecked. *** THE RULE TO TAKE AWAY, AND IT SHOULD "
   "CHANGE WHERE YOU LOOK FIRST: SEEK BITS THAT ARE CONSTANT BY SPECIFICATION BEFORE SEEKING BOUNDS THAT MIGHT BE "
   "TIGHTENED. *** The length field's low three bits are zero for EVERY valid input, unconditionally, because "
   "SHA-256 counts BITS and messages are whole bytes. That needs no growth argument, no carry analysis, no "
   "strict-inequality edge case — it is free and it CANNOT be unsound. Our four attempts chased a bound that had "
   "to be argued; the winning move took a fact that is simply true. SWEEP THE PADDING AND LENGTH ENCODING FOR "
   "OTHER SPEC-FIXED BITS before touching a numeric bound again."),
 ...
 "keccak-f1600": (307200, 184320, [
  ("122,880 — AND IT IS A THREE-RUNG LADDER WITH SMALL DIFFS, NOT THE MONOLITHIC PORT 414 JOBS HAVE FAILED "
   "AT. EVERY RUNG'S EXACT SCORE, FILE LIST AND SOURCE TREE IS RETRIEVABLE.","*** THE ENTIRE PUBLIC HISTORY "
   "OF THIS CHALLENGE IS FOUR SUBMISSIONS, ALL ON 2026-07-05 WITHIN 22 MINUTES, AND WE HAVE NEVER LANDED ONE. "
   "*** Read off the leaderboard API, which returns the FULL progression rather than one row per user:\n"
   "  284,160  bufferhe4d   d39ba663\n"
   "  230,400  thryec       f8fd1e03\n"
   "  199,680  thryec       20f52596\n"
   "  184,320  ordian       5bdce6d5  <- the record\n"
   "*** AND THE STEPS BETWEEN THEM ARE TINY. *** Fetched via "
   "GET /api/agent/v1/submissions/<id>/diff?base=<previous_id>, which returns a unified diff between any two "
   "public verified submissions:\n"
   "  284,160 -> 230,400  (-53,760)  43 KB, 11 files: Chi, ChiLane, Cost, KeccakRound, Main, MainTheorems, "
   "PermutationCost, PermutationDefs, Theta, ThetaC, Xor5Lane\n"
   "  230,400 -> 199,680  (-30,720)  24 KB,  8 files, and it ADDS EXACTLY ONE NEW FILE: Xor3Lane.lean\n"
   "  199,680 -> 184,320  (-15,360)  41 KB,  8 files: Cost, Main, PermutationCost, PermutationDefs, Theta, "
   "ThetaXor, Xor3Lane, Xor5Lane\n"
   "*** SO THE WHOLE 99,840 FROM 284,160 DOWN TO THE RECORD IS ABOUT 108 KB OF DIFF ACROSS ~27 FILE PAIRS, "
   "ALL IN ONE SUBSYSTEM: THETA PLUS THE XOR GADGETS. *** Chi is touched only in the first rung and never "
   "again, which is consistent with chi being at its proved floor. "
   "*** THE COST SPLITS CONFIRM THE r = m FLOOR AT EVERY RUNG, AND OUR SEED IS THE ONLY TREE THAT MISSES IT. "
   "*** 284,160 is 138,240 allocations + 145,920 constraints — UNEQUAL. Every rung after it is exactly equal: "
   "115,200+115,200, then 99,840+99,840, then 92,160+92,160. The first rung's real work is bringing rows down "
   "to witnesses; the rest is removing witnesses. Our seed is 153,600+153,600 = 307,200, i.e. already at r = m "
   "but 23,040 ABOVE the worst public tree. "
   "*** DO NOT ATTEMPT THE WHOLESALE PORT. *** The record tree has been sitting byte-identical in "
   "projs/keccak-f1600/reference/ for days and 414 jobs produced ZERO submissions against it. Take ONE RUNG. "
   "The cheapest and best-specified is the middle one: introduce Xor3Lane (the single-row XOR3 that this "
   "corpus has independently proved is unpublished elsewhere) and rewire Theta/ThetaC/Xor5Lane to use it. That "
   "is -30,720 on its own, one new file, and it lands you at 199,680 — which would be OUR FIRST EVER KECCAK "
   "SUBMISSION. *** ANY of these four scores beats our 307,200 seed, so even the WORST rung is a 23,040 "
   "improvement and a leaderboard entry. ***"),
