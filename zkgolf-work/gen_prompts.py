...
WORKQUEUE={
 "secp256k1-fixed-base-scalar-mul": (49218, 49536, [
  ("1344 — VERIFIED SAFE, DO THIS FIRST","PACK THE SELECTED y AS TWO 128-BIT HALVES. Confirmed from the circuits: y is a PURE AFFINE ADDEND everywhere — PairAddFoldA.stageAF (:118) multiplies only lam by dtilOf xT xA with yT/yA appearing solely inside FoldTail.rhsVecR, stageC1 (:154) is the same with yT2 only in foldRhsY, and stageX/stageO2 use xT only as an addend since x' = lam^2 - x - xT. So take C from 8 to 6 and Cy from 4 to 2: selectCost <266,266> -> <234,234>, total 48,252. IT DOES NOT MOVE THE WINDOW WIDTH (48250 at w=12 against 48288 at w=13), so do NOT retabulate. EDITS: replace the `nyn : fields 60` y witness with `fields 30` (halves at limb positions 0 and 2; limbs 1 and 3 become literal zero, hence free) and the `nyo : Emu` sign mux with a 2-wire mux; on the table side emit Borrow.half128 of magYNat instead of four limbs — PackEmu.packEmuOfNat IS ALREADY THAT FUNCTION, with value_packEmuOfNat proving BigInt.value unchanged; add twoPBorrowHalf : Fin 2 -> N beside twoPBorrowDigit with 2^128 <= D_h < 2^129 and D_0 + 2^128*D_1 = 2*P256, templated on the existing _ge/_lt/_sum lemmas; rehypothesise the chain from Fe.Valid to Fe.ValidW, for which PackedY.lean ALREADY has NormalizedP, Fe.ValidW, TValidP, NormAffP and ChainStartP/ChainExtP/ChainFinishP but is UNREACHABLE from Main — wire it in. Budget up to 80 for one extra carry bit at positions 0 and 2 of foldRhsY/rhsVecR. Roll up chainBody -> <1099,1103> and combCost -> <24081,24171>."),
  ("null result — do not spend on this","THE WINDOW WIDTH IS ALREADY OPTIMAL AT w = 12. A model calibrated to reproduce 49,596 with ZERO error sweeps w = 8..16 as 59750, 55826, 51906, 50802, 49596, 50140, 52590, 58462, 63606 — w=12 wins by 544 over 13 and 1206 over 11. The borrow-free negation did NOT move the optimum, since its 17 deleted units are window-count-linear. An earlier estimate of 48,072 at w=12 was wrong in level AND curvature. Non-uniform widths are legal but worth only 528 for a third gadget shape. STOP SPENDING SLOTS HERE."),
