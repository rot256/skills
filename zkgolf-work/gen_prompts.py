...
WORKQUEUE={
 ...
 "secp256k1-scalar-mul": (316436, None, [
  ...
  ("~10500","FREE TABLE ENTRIES FROM THE HORIZONTAL LINE — A REGROUPING THAT REVIVES THE CM COLLINEARITY THE PLAYBOOK PREVIOUSLY RECORDED AS INAPPLICABLE. For any R on secp256k1, R + phi(R) = (beta^2 x_R, -y_R) = -phi^2(R), because R, phi(R), phi^2(R) share a y and are the three intersections of a HORIZONTAL line, so the chord slope is exactly 0 and both output coordinates are FREE AFFINE. The naive use of this fails on our table because the two bases are independently signed — but this regrouping, verified over all 8 sign patterns, does not care: e1*P + e2*phi(P) + e3*Q + e4*phi(Q) = R_{e3} + e2*phi(R_{e2 e4}) with R_eps := P + eps*Q. Build R+ = P+Q and R- = P-Q at 3 general mulmods each, take phi(R+-) as CONSTANT-mulmods, and TWO OF THE EIGHT TABLE ENTRIES ARE THEN EXACTLY -phi^2(R+-) AND COST NOTHING. Modelled 24 general + 4 constant mulmods against 39 + 2. Soundness: R+- != O and R+- not in {+-phi(R+-)}, from P,Q independent and n prime. MEASURE our GLVBuildTable against this before building — glvBuildTableCost is currently 15430/15538."),
