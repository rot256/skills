STARTING POINT: `Solution/Secp256k1ScalarMulFixedBase/` contains the CURRENT LEADERBOARD RECORD for this
challenge — score = allocations+constraints = 14882824 (by gopikannappan) — and it ALREADY FULLY VERIFIES
(no `sorry`). 

GOAL: a SMALL, SAFE, GUARANTEED fully-proved reduction STRICTLY BELOW 14882824. Prioritize CERTAINTY
...
Classical.choice, and `Challenge.Instances.Secp256k1ScalarMulFixedBase.Interface.hCircomPrime`. Set `allocations`/
`constraints` to the true new cost and prove `mainCost`/`isR1CS`. Keep the build green (lean4
v4.28.0 / Mathlib v4.28.0). Return the complete updated `Solution/Secp256k1ScalarMulFixedBase/` files, fully compiling.

A tiny verified win that beats 14882824 is a success — do not overreach and leave anything unproved.
