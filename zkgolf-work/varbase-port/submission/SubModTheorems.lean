...
lemma lhs_boundsN (env : Environment (F circomPrime)) (i₀ : ℕ)
    (b_var : Var Emu (F circomPrime)) (b : Emu (F circomPrime))
    (h_input_b : Vector.map (Expression.eval env) b_var = b)
    (hb_norm : b.Normalized limbBits)
    (hr : ∀ k : ℕ, k < numLimbs → (env.get (i₀ + k)).val < 2 ^ limbBits) :
  ...
  · rw [dif_pos hk,
      ...
      show Expression.eval env (b_var[k.val]'hk) = b[k.val]'hk from by
        rw [← h_input_b, Vector.getElem_map]]
    exact AddMod.bound_add_limbN (hr k.val hk) (hb_norm ⟨k.val, hk⟩)
  · rw [dif_neg hk, show Expression.eval env (0 : Expression (F circomPrime)) = 0 from rfl,
      ZMod.val_zero]
    positivity


lemma rhs_boundsN (env : Environment (F circomPrime)) (i₀ : ℕ)
    (a_var : Var Emu (F circomPrime)) (a : Emu (F circomPrime))
    (h_input_a : Vector.map (Expression.eval env) a_var = a)
    (ha_norm : a.Normalized limbBits) (qN : ℕ)
    (hq : env.get (i₀ + numLimbs) = ((qN : ℕ) : F circomPrime)) (hqN : qN ≤ 1) :
  ...
  · rw [dif_pos hk,
    ...
    have h4 : qN * limbOfNat P256 k.val ≤ 1 * limbOfNat P256 k.val :=
      Nat.mul_le_mul_right _ hqN
    have := AddMod.limb_add_le_boundN
    omega
  · rw [dif_neg hk, show Expression.eval env (0 : Expression (F circomPrime)) = 0 from rfl,
      ZMod.val_zero]
    positivity
...
lemma soundness_coreN (i₀ : ℕ) (env : Environment (F circomPrime))
  ...
  have hq_cast : env.get (i₀ + numLimbs)
    ...
    (ZMod.natCast_zmod_val _).symm
  have h_polyeq := h_eq_impl
    ⟨lhs_boundsN env i₀ b_var b h_input_b hb_norm (outVar_val_lt env i₀ hr_norm),
      rhs_boundsN env i₀ a_var a h_input_a ha_norm _ hq_cast hq01⟩
  rw [polyValue_lhs env i₀ b_var b h_input_b hb_norm (outVar_val_lt env i₀ hr_norm),
    polyValue_rhs env i₀ a_var a h_input_a ha_norm _ hq_cast hq01] at h_polyeq
  have hr_lt : BigInt.value limbBits (Vector.map (Expression.eval env)
      (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) < P256 := by
...
lemma completeness_coreN (i₀ : ℕ) (env : Environment (F circomPrime))
  ...
  · rw [hrv_val, pConst_value env]
    exact hrN_lt
  · exact lhs_boundsN env i₀ b_var b h_input_b hb_norm (outVar_val_lt env i₀ hrv_norm)
  · exact rhs_boundsN env i₀ a_var a h_input_a ha_norm qNat h_wit_q hqNat_le
  · rw [polyValue_lhs env i₀ b_var b h_input_b hb_norm (outVar_val_lt env i₀ hrv_norm),
      polyValue_rhs env i₀ a_var a h_input_a ha_norm qNat h_wit_q hqNat_le,
      hrv_val]
    exact hkey
...
lemma sub3_witness_identity {va vb vc : ℕ}
    (hva : va < P256) (hvb : vb < P256) (hvc : vc < P256) :
    (va + 2 * P256 - vb - vc) % P256 + vb + vc
        = va + (2 - (va + 2 * P256 - vb - vc) / P256) * P256
      ∧ 2 - (va + 2 * P256 - vb - vc) / P256 ≤ 2 := by
  have hP : P256 = 115792089237316195423570985008687907853269984665640564039457584007908834671663 :=
    rfl
  rw [hP] at hva hvb hvc ⊢
  omega


lemma lhs_bounds3N (env : Environment (F circomPrime)) (i₀ : ℕ)
    (b_var c_var : Var Emu (F circomPrime)) (b c : Emu (F circomPrime))
    (h_input_b : Vector.map (Expression.eval env) b_var = b)
    (h_input_c : Vector.map (Expression.eval env) c_var = c)
    (hb_norm : b.Normalized limbBits) (hc_norm : c.Normalized limbBits)
    (hr : ∀ k : ℕ, k < numLimbs → (env.get (i₀ + k)).val < 2 ^ limbBits) :
    ∀ k : Fin (2 * 3 - 1),
      (Expression.eval env
          (if h : k.val < numLimbs
            then var { index := i₀ + k.val } + b_var[k.val]'h + c_var[k.val]'h else 0)).val
        < 2 ^ 66 := by
  intro k
  by_cases hk : k.val < numLimbs
  · rw [dif_pos hk,
      show Expression.eval env (var { index := i₀ + k.val } + b_var[k.val]'hk + c_var[k.val]'hk)
        = env.get (i₀ + k.val) + Expression.eval env (b_var[k.val]'hk)
    ...
    exact AddMod.bound_add3_limbN (hr k.val hk) (hb_norm ⟨k.val, hk⟩) (hc_norm ⟨k.val, hk⟩)
  · rw [dif_neg hk, show Expression.eval env (0 : Expression (F circomPrime)) = 0 from rfl,
      ZMod.val_zero]
    positivity


lemma rhs_bounds2q (env : Environment (F circomPrime)) (i₀ : ℕ)
    (a_var : Var Emu (F circomPrime)) (a : Emu (F circomPrime))
    (h_input_a : Vector.map (Expression.eval env) a_var = a)
    (ha_norm : a.Normalized limbBits) (qN : ℕ)
  ...
  · rw [dif_pos hk,
    ...
    have h2 := ZMod.val_add_le (a[k.val]'hk)
      (((qN : ℕ) : F circomPrime) * ((limbOfNat P256 k.val : ℕ) : F circomPrime))
    rw [AddMod.val_q_mul_limb2 hqN k.val] at h2
    have h3 := limbOfNat_lt P256 k.val
    have h4 : qN * limbOfNat P256 k.val ≤ 2 * limbOfNat P256 k.val :=
      Nat.mul_le_mul_right _ hqN
    have := AddMod.limb_add3_le_bound3N
    omega
  · rw [dif_neg hk, show Expression.eval env (0 : Expression (F circomPrime)) = 0 from rfl,
      ZMod.val_zero]
    positivity
...
lemma polyValue_lhs3 (env : Environment (F circomPrime)) (i₀ : ℕ)
    (b_var c_var : Var Emu (F circomPrime)) (b c : Emu (F circomPrime))
    (h_input_b : Vector.map (Expression.eval env) b_var = b)
    (h_input_c : Vector.map (Expression.eval env) c_var = c)
    (hb_norm : b.Normalized limbBits) (hc_norm : c.Normalized limbBits)
    ...
    polyValue limbBits (Vector.map (Expression.eval env)
        (Vector.mapFinRange (2 * 3 - 1) fun k =>
          if h : k.val < numLimbs
          then var { index := i₀ + k.val } + b_var[k.val]'h + c_var[k.val]'h else 0))
      = BigInt.value limbBits (Vector.map (Expression.eval env)
          (Vector.mapRange numLimbs fun i => var { index := i₀ + i }))
        + BigInt.value limbBits b + BigInt.value limbBits c := by
  refine (polyValue_padded limbBits env
      (fun k h => var { index := i₀ + k.val } + b_var[k.val]'h + c_var[k.val]'h)
      (fun k => (env.get (i₀ + k)).val
        + (if h : k < numLimbs then (b[k]'h).val else 0)
        + (if h : k < numLimbs then (c[k]'h).val else 0)) ?_).trans ?_
  · intro k hk
    rw [show Expression.eval env (var { index := i₀ + k.val } + b_var[k.val]'hk + c_var[k.val]'hk)
        = env.get (i₀ + k.val) + Expression.eval env (b_var[k.val]'hk)
          + Expression.eval env (c_var[k.val]'hk) from rfl,
      show Expression.eval env (b_var[k.val]'hk) = b[k.val]'hk from by
        rw [← h_input_b, Vector.getElem_map],
      show Expression.eval env (c_var[k.val]'hk) = c[k.val]'hk from by
        rw [← h_input_c, Vector.getElem_map],
      AddMod.val_add3_limb (u := env.get (i₀ + k.val)) (v := b[k.val]'hk) (w := c[k.val]'hk)
        (hr k.val hk) (hb_norm ⟨k.val, hk⟩) (hc_norm ⟨k.val, hk⟩)]
    simp only [dif_pos hk]
  · rw [value_outVar limbBits env i₀, value_eq_range_sum limbBits b,
...
lemma polyValue_rhs2q (env : Environment (F circomPrime)) (i₀ : ℕ)
    (a_var : Var Emu (F circomPrime)) (a : Emu (F circomPrime))
    (h_input_a : Vector.map (Expression.eval env) a_var = a)
    (ha_norm : a.Normalized limbBits) (qN : ℕ)
    (hq : env.get (i₀ + numLimbs) = ((qN : ℕ) : F circomPrime)) (hqN : qN ≤ 2) :
  ...
  · intro k hk
    rw [show Expression.eval env
      ...
      show Expression.eval env (a_var[k.val]'hk) = a[k.val]'hk from by
        rw [← h_input_a, Vector.getElem_map],
      hq, eval_pConst_getElem env k.val hk,
      ZMod.val_add_of_lt, AddMod.val_q_mul_limb2 hqN k.val]
    · simp only [dif_pos hk]
    · rw [AddMod.val_q_mul_limb2 hqN k.val]
      have h1 : (a[k.val]'hk).val < 2 ^ limbBits := ha_norm ⟨k.val, hk⟩
      have h2 := limbOfNat_lt P256 k.val
      have h3 : qN * limbOfNat P256 k.val ≤ 2 * limbOfNat P256 k.val :=
        Nat.mul_le_mul_right _ hqN
      have := AddMod.limb_add3_lt
      omega
  · rw [value_eq_range_sum limbBits a]
    have hsplit : (∑ k ∈ Finset.range numLimbs,
          ((if h : k < numLimbs then (a[k]'h).val else 0) + qN * limbOfNat P256 k)
...
lemma soundness_core3N (i₀ : ℕ) (env : Environment (F circomPrime))
    ...
    (h_eq_impl :
      ((∀ k : Fin (2 * 3 - 1),
          (Expression.eval env
              (if h : k.val < numLimbs
                then var { index := i₀ + k.val } + b_var[k.val]'h + c_var[k.val]'h
                else 0)).val
            < 2 ^ 66) ∧
        ∀ k : Fin (2 * 3 - 1),
          (Expression.eval env
  ...
  have hq_cast : env.get (i₀ + numLimbs)
    ...
    (ZMod.natCast_zmod_val _).symm
  have h_polyeq := h_eq_impl
    ⟨lhs_bounds3N env i₀ b_var c_var b c h_input_b h_input_c hb_norm hc_norm
        (outVar_val_lt env i₀ hr_norm),
      rhs_bounds2q env i₀ a_var a h_input_a ha_norm _ hq_cast hq2⟩
  rw [polyValue_lhs3 env i₀ b_var c_var b c h_input_b h_input_c hb_norm hc_norm
      (outVar_val_lt env i₀ hr_norm),
    polyValue_rhs2q env i₀ a_var a h_input_a ha_norm _ hq_cast hq2] at h_polyeq
  have hr_lt : BigInt.value limbBits (Vector.map (Expression.eval env)
      (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) < P256 := by
    have h := h_lt_impl ⟨hr_norm, pConst_normalized env⟩
    rwa [pConst_value env] at h
...
lemma completeness_core3N (i₀ : ℕ) (env : Environment (F circomPrime))
    ...
    (h_wit_r : ∀ i : Fin numLimbs,
      env.get (i₀ + i.val)
        = (emuOfNat ((BigInt.value limbBits a + 2 * P256 - BigInt.value limbBits b
            - BigInt.value limbBits c) % P256))[i.val])
    (h_wit_q : env.get (i₀ + numLimbs)
      = (((2 - (BigInt.value limbBits a + 2 * P256 - BigInt.value limbBits b
          - BigInt.value limbBits c) / P256 : ℕ)) : F circomPrime)) :
    ((2 - (BigInt.value limbBits a + 2 * P256 - BigInt.value limbBits b
        - BigInt.value limbBits c) / P256 : ℕ) ≤ 2) ∧
      BigInt.Normalized limbBits (Vector.map (Expression.eval env)
          (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) ∧
        ((BigInt.Normalized limbBits (Vector.map (Expression.eval env)
              (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) ∧
  ...
  set qNat : ℕ := 2 - (va + 2 * P256 - vb - vc) / P256 with hqNat
  set rN := (va + 2 * P256 - vb - vc) % P256 with hrN
  have hqNat_le : qNat ≤ 2 := by
    rw [hqNat]; exact Nat.sub_le 2 _
  have hrN_lt : rN < P256 := Nat.mod_lt _ P256_pos
  have hrN_pow : rN < 2 ^ (limbBits * numLimbs) := lt_trans hrN_lt P256_lt
  have hkey' := sub3_witness_identity ha_lt hb_lt hc_lt
  have hkey : rN + vb + vc = va + qNat * P256 := hkey'.1
  have hwit : ∀ i : Fin numLimbs, env.get (i₀ + i.val)
      = ((rN / 2 ^ (limbBits * i.val) % 2 ^ limbBits : ℕ) : F circomPrime) := by
    intro i
    rw [h_wit_r i, emuOfNat_getElem _ i.val i.isLt]
  ...
  have hrv_val : BigInt.value limbBits (Vector.map (Expression.eval env)
      (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) = rN :=
    BigInt.value_mapRange i₀ rN env two_pow_limb_lt hrN_pow hwit
  refine ⟨hqNat_le, hrv_norm, ⟨⟨hrv_norm, pConst_normalized env⟩, ?_⟩, ⟨?_, ?_⟩, ?_⟩
  · rw [hrv_val, pConst_value env]
    exact hrN_lt
  · exact lhs_bounds3N env i₀ b_var c_var b c h_input_b h_input_c hb_norm hc_norm
      (outVar_val_lt env i₀ hrv_norm)
  · exact rhs_bounds2q env i₀ a_var a h_input_a ha_norm qNat h_wit_q hqNat_le
  · rw [polyValue_lhs3 env i₀ b_var c_var b c h_input_b h_input_c hb_norm hc_norm
        (outVar_val_lt env i₀ hrv_norm),
      polyValue_rhs2q env i₀ a_var a h_input_a ha_norm qNat h_wit_q hqNat_le,
      hrv_val]
    exact hkey
