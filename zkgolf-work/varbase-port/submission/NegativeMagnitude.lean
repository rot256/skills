...
def asEmu (x : Expression (F circomPrime)) : Var Emu (F circomPrime) :=
  #v[x, 0, 0, 0]
...
theorem affineW_asEmu (x : Expression (F circomPrime)) (hx : Affine x) :
    AffineW (NegativeMagnitude.asEmu x) := by
  intro i hi
  interval_cases i
  · simpa [NegativeMagnitude.asEmu] using hx
