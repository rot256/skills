import Solution.Secp256k1ScalarMul.MulModFold32M
...
def mainInv (input : Var Inputs (F circomPrime)) : Circuit (F circomPrime) Unit := do
  ...
  assertion (RangeCheck.circuit qBitsFold32M (by decide) (by decide))
    (qInv Pc input.target)
  assertion
    (GroupedEqXV.circuitNoTop 32 gfFold32M posOfFold32M 3 vFold32ML vFold32MR
      hgvFold32M (by norm_num))
...
instance elaboratedInv : ElaboratedCircuit (F circomPrime) Inputs unit mainInv where
  ...
  localLength_eq := by
    ...
    simp only [mainInv, interpolatedMul14, circuit_norm, RangeCheck.circuit,
      GroupedEqXV.circuitNoTop, GroupedEqXV.elaboratedNoTop, vFold32ML, wfFold32M,
...
attribute [local irreducible] interpolatedMul14 RangeCheck.circuit
  GroupedEqXV.circuitNoTop GroupedEqXV.mainNoTop
  qInv lhsPolyInv targetPolyInv

set_option maxHeartbeats 8000000 in
theorem soundnessInv (Ca Cb : ℕ) (hcap : 4 * (Ca * Cb) ≤ (3 * 2 ^ 99)) :
  ...
  have hqv : (Expression.eval env
      (qInv (interpolatedMul14 input_var_a input_var_b i₀).1 input_var_target)).val < 2 ^ 69 :=
    hq_lt
  have heq := h_eq_impl
...
set_option maxHeartbeats 8000000 in
theorem completenessInv (Ca Cb : ℕ) (hcap : 4 * (Ca * Cb) ≤ (3 * 2 ^ 99)) :
  ...
  have hqvar : (Expression.eval env.toEnvironment
      (qInv (interpolatedMul14 input_var_a input_var_b i₀).1 input_var_target)).val < 2 ^ 69 := by
    rw [hqval]
    exact hqlt
...
def circuitInv (Ca Cb : ℕ) (hcap : 4 * (Ca * Cb) ≤ (3 * 2 ^ 99)) :
...
theorem computableWitnessesInv (Ca Cb : ℕ) (hcap : 4 * (Ca * Cb) ≤ (3 * 2 ^ 99)) :
  ...
  · exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      ...
      (by
        ...
        rw [hL, hR])
      (GroupedEqXV.computableWitnessesNoTop 32 gfFold32M posOfFold32M 3
        vFold32ML vFold32MR hgvFold32M (by norm_num))
      env env'
