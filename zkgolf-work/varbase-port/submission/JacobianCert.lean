...
lemma rawDistinct_of_bool {P Q : Raw} (h : rawDistinctB P Q = true) :
    rawDistinct P Q := by
  simp only [rawDistinctB, Bool.or_eq_true, decide_eq_true_eq] at h
  exact h
...
set_option maxRecDepth 100000 in
example : coeff 128 = (a, bAbs) := by decide
