...
attribute [local irreducible] main PointValid.main GLVScalarRelation.main
  GLVBuildTable.main GLVBuildTable.tableAt GLVMSM.main AssertInfinity.main
...
theorem assertInfinityComputableWitnesses :
    AssertInfinity.circuit.ComputableWitnesses := by
  intro offset input env env'
  change Operations.forAllFlat offset
    (FormalCircuitBase.computableWitnessCondition
      input env env') ((AssertInfinity.main input).operations offset)
  apply FormalCircuitBase.Operations.forAllFlat_of_structuralComputableWitnesses
  unfold AssertInfinity.main
  simp only [
