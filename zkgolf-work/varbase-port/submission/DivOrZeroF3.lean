...
import Solution.Secp256k1ScalarMul.MulModTargetT3
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  -- λ is normalized; canonicality is not needed because every consumer uses λ
  -- only as a multiplication operand, where the limb bound is what matters.
...
def Spec (input : Inputs (F circomPrime)) (lam : Emu (F circomPrime)) : Prop :=
  BigInt.Normalized limbBits lam ∧
    (decodeFe input.den ≠ 0 →
      decodeFe lam * decodeFe input.den =
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  ...
  obtain ⟨hz, hAB_ops, hT, hlam_valid, hmul⟩ := h_holds
  specialize hz h_den_limb3
  simp only [MulMod.interpolatedMul_localLength] at hT hlam_valid hmul
  set zc := env.get (i₀ + 1) with hzc_def
  set denSafeVar := denSafeVec input_var_den (var (F := F circomPrime) { index := i₀ + 1 })
    with hdenSafeVar
  set off_ip := i₀ + 2 with hoff_ip
  ...
  set lamVar := Vector.mapRange numLimbs fun i =>
  ...
  have hden : Vector.map (Expression.eval env) denSafeVar
      = if zc = 1 then emuOfNat 1 else input_den := by
    rw [hdenSafeVar,
      eval_denSafeVec env input_var_den (var (F := F circomPrime) { index := i₀ + 1 })
  ...
  have hspec_of : polyValue limbBits (Vector.map (Expression.eval env) TVar) % P256
    ...
    hmul ⟨fun i => by
        simpa [hlamVar, Vector.getElem_map, Vector.getElem_mapRange] using hlam_valid i,
...
theorem completeness : Completeness (F circomPrime) main Assumptions := by
  ...
  have hlam_eval : Vector.map (Expression.eval env.toEnvironment) lamVar
      = emuOfNat (ZMod.val ((if input_sel = 1
    ...
    apply Vector.ext
    intro k hk
    have hentry : (Vector.map (Expression.eval env.toEnvironment) lamVar)[k]'hk
        = env.get (off_ip + (2 * numLimbs - 1) + (2 * numLimbs - 1) + k) := by
      rw [hlamVar, Vector.getElem_map, Vector.getElem_mapRange]; rfl
    rw [hentry]
  ...
  have hlam_norm : BigInt.Normalized limbBits
    ...
    rw [hlam_eval]; exact emuOfNat_normalized _
  have hlam_val : BigInt.value limbBits (Vector.map (Expression.eval env.toEnvironment) lamVar)
    ...
    rw [hlam_eval]; exact value_emuOfNat (lt_trans hq_lt P256_lt)
  ...
  exact ⟨h_den_limb3,
    ...
    hlam_norm,
    ⟨⟨fun i => by
        simpa [hlamVar, Vector.getElem_map, Vector.getElem_mapRange] using hlam_norm i,
...
theorem computableWitnesses : circuit.base.ComputableWitnesses := by
  ...
  have hvp : ∀ (v : Var Emu (F circomPrime)) (o : ℕ),
      (Normalize.circuit secpParams v).localLength o = 252 := fun _ _ => rfl
  ...
  simp only [
    ...
    hz, hmxT, hip, hw, hvp, and_true]
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · -- z ← IsZeroFeSum den
    exact Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses
      (Parent := Inputs) IsZeroFeSum.circuit _ den offset
      (fun e e' h => by
        have hden := congrArg (fun v : Inputs (F circomPrime) => v.den) h
      ...
      { selector := sel,
        ifTrue := Vector.mapFinRange (2 * numLimbs - 1) fun k =>
          ((3 : F circomPrime) : Expression (F circomPrime))
            * ((MulMod.interpolatedMul x x).output (offset + 2))[k.val],
        ifFalse := Vector.mapFinRange (2 * numLimbs - 1) fun k =>
          if h : k.val < numLimbs then dyU[k.val]'h else (0 : Expression (F circomPrime)) }
      (offset + 2 + (2 * numLimbs - 1))
      (by
        ...
        have hTT_eq : Vector.map (Expression.eval e.toEnvironment)
              (Vector.mapFinRange (2 * numLimbs - 1) fun j =>
                ((3 : F circomPrime) : Expression (F circomPrime))
                  * ((MulMod.interpolatedMul x x).output (offset + 2))[j.val])
            = Vector.map (Expression.eval e'.toEnvironment)
              (Vector.mapFinRange (2 * numLimbs - 1) fun j =>
                ((3 : F circomPrime) : Expression (F circomPrime))
                  * ((MulMod.interpolatedMul x x).output (offset + 2))[j.val]) := by
          apply Vector.ext
          intro i hi
  ...
  · -- Sparse canonical validation of λ
    exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      ...
      ((ProvableType.witness (α := Emu) fun env =>
        ...
        emuOfNat (numFp * denFp⁻¹).val).output
          (offset + 2 + (2 * numLimbs - 1) + (2 * numLimbs - 1)))
      _
      (by
        intro k e e' hle h_agree _
        exact AddMod.emuWitnessOutput_stable _ h_agree
          (offset := offset + 2 + (2 * numLimbs - 1) + (2 * numLimbs - 1))
          (by omega))
  ...
  · -- MulModFoldT { λ, denSafe, T }
    exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      ...
      { a := (ProvableType.witness (α := Emu) fun env =>
          ...
          emuOfNat (numFp * denFp⁻¹).val).output
            (offset + 2 + (2 * numLimbs - 1) + (2 * numLimbs - 1)),
        b := denSafeVec den ((subcircuit IsZeroFeSum.circuit den).output offset),
      ...
      (by
        intro k e e' hle h_agree h_in
        have hlam := AddMod.emuWitnessOutput_stable
          (fun env =>
            let denFp : Specs.Secp256k1.Fp :=
              ((if evalEmu env den = 0 then 1 else evalEmu env den : ℕ) : Specs.Secp256k1.Fp)
            let numFp : Specs.Secp256k1.Fp :=
        ...
        have hdenMap : Vector.map (Expression.eval e.toEnvironment)
              (denSafeVec den ((subcircuit IsZeroFeSum.circuit den).output offset))
            = Vector.map (Expression.eval e'.toEnvironment)
              (denSafeVec den ((subcircuit IsZeroFeSum.circuit den).output offset)) := by
          have hsel := IsZeroFeSum.eval_output_of_agreesBelow den (offset := offset) h_agree
            (by omega)
        ...
        have hTO := Mux.eval_output_of_agreesBelow (M := fields (2 * numLimbs - 1))
          ...
          h_agree (offset := offset + 2 + (2 * numLimbs - 1))
          (k := k) (by omega)
        simp only [circuit_norm]
        rw [MulModFoldT.Inputs.mk.injEq]
        exact ⟨emu_map_eval_eq_of_eval_eq hlam, hdenMap,
...
lemma eval_output_of_agreesBelow (input : Var Inputs (F circomPrime)) {offset k : ℕ}
    ...
    (hk : offset + 2 + (2 * numLimbs - 1) + (2 * numLimbs - 1)
        + numLimbs ≤ k) :
  ...
  have hout : (main ⟨sel, x, dyU, den⟩).output offset
      = (ProvableType.witness (α := Emu) fun env =>
          ...
          emuOfNat (numFp * denFp⁻¹).val).output
        (offset + 2
          + (MulMod.interpolatedMul x x).localLength (offset + 2)
          + (2 * numLimbs - 1)) := rfl
  have hlen : (MulMod.interpolatedMul x x).localLength (offset + 2)
      = 2 * numLimbs - 1 :=
    MulMod.interpolatedMul_localLength _ _ _
  rw [hout, hlen]
  exact AddMod.emuWitnessOutput_stable _ h_agree (by omega)
