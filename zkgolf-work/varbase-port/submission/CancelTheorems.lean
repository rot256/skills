...
lemma cancel_flag_equiv
    {P Q : FlaggedPoint (F circomPrime)}
    (hP : P.Valid) (hQ : Q.Valid) (hPf : P.isInf = 0) (hQf : Q.isInf = 0) :
    (if decodeFe Q.x = decodeFe P.x then (1 : F circomPrime) else 0) *
      (1 - if decodeFe Q.y = decodeFe P.y then (1 : F circomPrime) else 0) =
    (if decodeFe Q.x = decodeFe P.x then (1 : F circomPrime) else 0) *
      (if decodeFe P.y + decodeFe Q.y = 0 then (1 : F circomPrime) else 0) := by
  by_cases hx : decodeFe Q.x = decodeFe P.x
  · rw [if_pos hx, one_mul, one_mul]
    ...
    · rw [if_pos hy]
      have hnot : decodeFe P.y + decodeFe Q.y ≠ 0 := by
        ...
        have hy0 : decodeFe P.y = 0 := by
          ...
          apply (mul_eq_zero.mp (by
            simpa [two_mul] using htwo)).resolve_left h2
        exact ProofBlocker.noOrderTwo _ (hP.2.2.2 hPf) hy0
      rw [if_neg hnot]
      ring
    · rw [if_pos ho]
      have hne : decodeFe Q.y ≠ decodeFe P.y := by
        intro heq
        rw [heq] at ho
        have hy0 : decodeFe P.y = 0 := by
          have h2 : (2 : Fp) ≠ 0 := by decide
          apply (mul_eq_zero.mp (by
            simpa [two_mul] using ho)).resolve_left h2
        exact ProofBlocker.noOrderTwo _ (hP.2.2.2 hPf) hy0
      rw [if_neg hne]
      ring
  · rw [if_neg hx, zero_mul, zero_mul]
