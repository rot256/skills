...
namespace EqViaCarriesN


structure NParams (p : ℕ) where
  ...
  hWp : C * 3 + 2 ^ W * 2 ^ B + 2 ^ W < p


def main (N : NParams p) [Fact (p > 2)]
  ...
  let carry ← witnessVector (2 * m - 1) fun env =>
    Vector.ofFn fun k : Fin (2 * m - 1) =>
      ((N.OFF + evalPartial N.B env Pc k.val / 2 ^ (N.B * (k.val + 1))
          - evalPartial N.B env Sc k.val / 2 ^ (N.B * (k.val + 1)) : ℕ) : F p)

  -- 2. range-check each carry to `W` bits (subcircuit call).
  Circuit.forEach carry (fun c => Gadgets.ToBits.rangeCheck N.W N.hW c)

  -- 3. per-index linear constraint (signed carries via the offset convention).
  let constraints : Vector (Expression (F p)) (2 * m - 1) :=
    Vector.mapFinRange (2 * m - 1) fun k =>
      let carryIn : Expression (F p) :=
        if h : k.val = 0 then 0 else carry[k.val - 1]'(by omega) - (N.OFF : F p)
      Pc[k.val] + carryIn - Sc[k.val]
        - (carry[k.val] - (N.OFF : F p)) * (2 ^ N.B : F p)
  Circuit.forEach constraints assertZero

  -- 4. force the top (offset) carry to OFF, i.e. signed top carry to zero.
  if h : 2 * m - 1 = 0 then pure () else
    assertZero (carry[2 * m - 1 - 1]'(by omega) - (N.OFF : F p))

instance elaborated (N : NParams p) [Fact (p > 2)] :
    ElaboratedCircuit (F p) (EqViaCarries.Inputs m) unit (main N) where
  localLength _ := (2 * m - 1) * N.W + (2 * m - 1)
  localLength_eq := by
    intro input offset
    simp only [main, circuit_norm, Gadgets.ToBits.rangeCheck]
    split <;> simp +arith [circuit_norm]
...
def Assumptions (C : ℕ) (input : EqViaCarries.Inputs m (F p)) : Prop :=
  (∀ k : Fin (2 * m - 1), (input.lhs[k.val]).val < C) ∧
  (∀ k : Fin (2 * m - 1), (input.rhs[k.val]).val < C)


def Spec (B : ℕ) (input : EqViaCarries.Inputs m (F p)) : Prop :=
  polyValue B input.lhs = polyValue B input.rhs


def circuit (N : NParams p) [Fact (p > 2)] : FormalAssertion (F p) (EqViaCarries.Inputs m) where
    main := main N
    Assumptions := Assumptions N.C
    Spec := Spec N.B
    soundness := by
      obtain ⟨B, W, C, OFF, hB, hW, hB1, hC1, hOW, hCO, hWp⟩ := N
      circuit_proof_start
      simp only [circuit_norm, Gadgets.ToBits.rangeCheck] at h_holds ⊢
      obtain ⟨h_range, h_lin, h_top⟩ := h_holds
      refine ⟨?_, by split <;> simp [circuit_norm]⟩
      ...
      have hCn_lt : ∀ k, k < 2 * m - 1 → Cn k < 2 ^ W := by
        intro k hk; simpa [hCn] using h_range ⟨k, hk⟩
      have hPn_lt : ∀ k, k < 2 * m - 1 → Pn k < C := by
        intro k hk; simp only [hPn, dif_pos hk]; exact h_assumptions.1 ⟨k, hk⟩
      have hSn_lt : ∀ k, k < 2 * m - 1 → Sn k < C := by
        intro k hk; simp only [hSn, dif_pos hk]; exact h_assumptions.2 ⟨k, hk⟩
      -- standing bounds on OFFn and 2^B, derived from hOW / hCO / hWp
      have hOFFn_le_W : OFFn ≤ 2 ^ W := by omega
      have hOFFB_le : OFFn * 2 ^ B ≤ 2 ^ W * 2 ^ B :=
        Nat.mul_le_mul_right _ hOFFn_le_W
      have hOFFB_lt : OFFn * 2 ^ B < p := by omega
      have hpB : 2 ^ B < p := by
        have : 2 ^ B ≤ 2 ^ W * 2 ^ B := Nat.le_mul_of_pos_left _ (Nat.two_pow_pos W)
        omega
      have hOFFn_lt : OFFn < p := by omega
      have hOFFn_cast : (OFFn : F p).val = OFFn := ZMod.val_natCast_of_lt hOFFn_lt
      -- X := C ≤ 2^W * 2^B  (offset-fits route: C ≤ OFF·(2^B−1) ≤ OFF·2^B ≤ 2^W·2^B)
      have hXW : C ≤ 2 ^ W * 2 ^ B := by
        have h1 : OFFn * (2 ^ B - 1) ≤ OFFn * 2 ^ B :=
    ...
    completeness := by
      ...
      set Pn : ℕ → ℕ := fun k => if h : k < 2 * m - 1 then (input.lhs[k]'h).val else 0 with hPn
      set Sn : ℕ → ℕ := fun k => if h : k < 2 * m - 1 then (input.rhs[k]'h).val else 0 with hSn
      have hPn_lt : ∀ k, Pn k < C := by
        intro k; simp only [hPn]; split
        · rename_i h; exact h_assumptions.1 ⟨k, h⟩
        · omega
      have hSn_lt : ∀ k, Sn k < C := by
        intro k; simp only [hSn]; split
        · rename_i h; exact h_assumptions.2 ⟨k, h⟩
        · omega
      -- partial sums
      ...
      have hwit_eq : ∀ k, k < 2 * m - 1 → env.get (i₀ + k) = (Cn k : F p) := by
        ...
        rw [h_wit ⟨k, hk⟩]
        simp only [Vector.getElem_ofFn, hCn_app, hDk_app, hPFn_eq, hPSn_eq]
      -- per-sequence div bounds (carry magnitude), via the sharp cap bound
      have hPFdiv : ∀ k, PFn k / Dk k ≤ OFFn := by
        intro k; exact partial_div_boundC B Pn C OFFn hPn_lt hCO k
      have hPSdiv : ∀ k, PSn k / Dk k ≤ OFFn := by
        intro k; exact partial_div_boundC B Sn C OFFn hSn_lt hCO k
      -- range check: each carry < 2^W
      have hrange : ∀ k, Cn k < 2 ^ W := by
        intro k
        have h1 := hPFdiv k
...
theorem computableWitnesses (N : NParams p) [Fact (p > 2)] :
    (circuit (m := m) N).ComputableWitnesses := by
  ...
  · intro _ h_input
    ...
    have hlhs :
        evalPartial N.B env input.lhs i =
          evalPartial N.B env' input.lhs i := by
        ...
        have hfield :
          ...
          simpa [circuit_norm, CircuitType.eval_expression_prover_to_verifier,
            ...
            Vector.getElem_map] using
              congrArg (fun x : EqViaCarries.Inputs m (F p) => x.lhs[j]'hj) h_input
        exact congrArg ZMod.val hfield
      · simp only [dif_neg hj]
    have hrhs :
        evalPartial N.B env input.rhs i =
          evalPartial N.B env' input.rhs i := by
      simp only [evalPartial]
      apply Finset.sum_congr rfl
      intro j _
      congr 1
      ...
      · simp only [dif_pos hj]
        have hfield :
          ...
          simpa [circuit_norm, CircuitType.eval_expression_prover_to_verifier,
            CircuitType.eval_expression, ProvableType.eval, explicit_provable_type,
            Vector.getElem_map] using
              congrArg (fun x : EqViaCarries.Inputs m (F p) => x.rhs[j]'hj) h_input
        exact congrArg ZMod.val hfield
      · simp only [dif_neg hj]
    rw [hlhs, hrhs]
  · intro i
    ...
    · intro k env env' hk h_agree _
      ...
      rw [show eval env.toEnvironment
              ((witnessVector (2 * m - 1) fun env =>
                Vector.ofFn fun k : Fin (2 * m - 1) =>
                  ((N.OFF + evalPartial N.B env input.lhs k.val / 2 ^ (N.B * (k.val + 1))
                      - evalPartial N.B env input.rhs k.val / 2 ^ (N.B * (k.val + 1)) : ℕ) :
