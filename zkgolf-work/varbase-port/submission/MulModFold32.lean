import Solution.Secp256k1ScalarMul.ParamsFold32
...
/-!
# Folded modular-multiplication certificate on 32-bit limbs

Certifies `target ≡ a·b (mod p)` for `p = 2^256 − 2^32 − 977`, where the two
operands are given as **eight 32-bit limbs** and the target as the usual four
64-bit limbs (placed at the even 32-bit positions, which is free).

Why 32-bit operands: `2^256 ≡ cFold = 2^32 + 977`, and in base `2^32` that
constant is the two *tiny* digits `977` and `1`, so the fold

  `d_k = c_k + 977·c_{k+8} + c_{k+7}`

only inflates a product cell by a factor `979` instead of `2^32`.  The quotient
is therefore a single 39-bit wire (against 69) and the grouped equality needs a
single 48-bit carry (against 101).  The 15-cell convolution costs 8 rows more
than the 7-cell one, so the certificate is 101/103 against 176/178.
-/
...
/-- Number of nonzero terms in the `k`-th convolution cell of two 8-limb operands. -/
def triCnt (k : ℕ) : ℕ := ∑ i : Fin 8, if i.val ≤ k ∧ k - i.val < 8 then 1 else 0

/-- Triangular cap on the `k`-th convolution cell when both operands are 32-bit. -/
def triNf (k : ℕ) : ℕ := triCnt k * 2 ^ 66
...
lemma val_lhs_digit {Nx Ny Nz M : ℕ} {x y z : F circomPrime} {d : ℕ}
    ...
    (hM : Nx + 977 * Ny + Nz + 3 * 2 ^ 32 ≤ M) (hMp : M ≤ 2 ^ 79) :
    (x + ((977 : ℕ) : F circomPrime) * y + z + ((d : ℕ) : F circomPrime)).val
        = x.val + 977 * y.val + z.val + d ∧
      x.val + 977 * y.val + z.val + d < M := by
  have hp79 : (2 : ℕ) ^ 79 < circomPrime := by decide
  have hsum : x.val + 977 * y.val + z.val + d < M := by omega
...
lemma val_rhs_digit {q t : F circomPrime} {k : ℕ}
    (hq : q.val < 2 ^ 39) (ht : t.val < 3 * 2 ^ 64) :
    (q * ((pNat32 k : ℕ) : F circomPrime) + t).val = q.val * pNat32 k + t.val ∧
      q.val * pNat32 k + t.val < (2 ^ 71 + 3 * 2 ^ 64) := by
  have hpk := pNat32_lt k
  have hmulb : q.val * pNat32 k < 2 ^ 71 := by
    calc q.val * pNat32 k < 2 ^ 39 * 2 ^ 32 :=
          Nat.mul_lt_mul_of_lt_of_le hq (by omega) (by positivity)
      _ = 2 ^ 71 := by norm_num
  have hsum : q.val * pNat32 k + t.val < (2 ^ 71 + 3 * 2 ^ 64) := by
    have h1 : (3 : ℕ) * 2 ^ 64 + 2 ^ 71 ≤ (2 ^ 71 + 3 * 2 ^ 64) := by decide
    omega
...
lemma val_rhs_digit_odd {q : F circomPrime} {k : ℕ} (hq : q.val < 2 ^ 39) :
    (q * ((pNat32 k : ℕ) : F circomPrime)).val = q.val * pNat32 k ∧
      q.val * pNat32 k < (2 ^ 71 + 3 * 2 ^ 64) := by
  have hpk := pNat32_lt k
  have hmulb : q.val * pNat32 k < 2 ^ 71 := by
    calc q.val * pNat32 k < 2 ^ 39 * 2 ^ 32 :=
          Nat.mul_lt_mul_of_lt_of_le hq (by omega) (by positivity)
...
lemma val_lhs_digit_two {Nx Ny M : ℕ} {x y : F circomPrime} {d : ℕ}
    (hx : x.val < Nx) (hy : y.val < Ny) (hd : d < 3 * 2 ^ 32)
    (hM : Nx + 977 * Ny + 3 * 2 ^ 32 ≤ M) (hMp : M ≤ 2 ^ 79) :
    (x + ((977 : ℕ) : F circomPrime) * y + ((d : ℕ) : F circomPrime)).val
        = x.val + 977 * y.val + d ∧
      x.val + 977 * y.val + d < M := by
  have hp79 : (2 : ℕ) ^ 79 < circomPrime := by decide
  have hsum : x.val + 977 * y.val + d < M := by omega
  have hmul : ((((977 : ℕ)) : F circomPrime) * y).val = 977 * y.val :=
    val_natCast_mul (by decide) (by omega)
  have hdv : (((d : ℕ) : F circomPrime)).val = d := ZMod.val_natCast_of_lt (by omega)
  refine ⟨?_, hsum⟩
...
lemma foldLhs32_bound (env : Environment (F circomPrime))
    ...
    (hPv : ∀ k : Fin 15, (Expression.eval env Pv[k.val]).val < triNf k.val) :
    ∀ k : Fin 8, (Expression.eval env (foldLhs32 Pv)[k.val]).val < nfFold32L k.val := by
  intro k
  ...
  fin_cases k
  · have hd := val_lhs_digit_two (M := nfFold32L 0) h0 h8 (offNat32_lt 0) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[0]).val < nfFold32L 0
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 1) h1 h9 h8 (offNat32_lt 1) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[1]).val < nfFold32L 1
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 2) h2 h10 h9 (offNat32_lt 2) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[2]).val < nfFold32L 2
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 3) h3 h11 h10 (offNat32_lt 3) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[3]).val < nfFold32L 3
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 4) h4 h12 h11 (offNat32_lt 4) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[4]).val < nfFold32L 4
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 5) h5 h13 h12 (offNat32_lt 5) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[5]).val < nfFold32L 5
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32L 6) h6 h14 h13 (offNat32_lt 6) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[6]).val < nfFold32L 6
    ...
    exact hd.2
  · have hd := val_lhs_digit_top (M := nfFold32L 7) h7 h14 (offNat32_lt 7) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[7]).val < nfFold32L 7
...
lemma foldLhs32_polyValue (env : Environment (F circomPrime))
  ...
  have h13 := hPv ⟨13, by decide⟩
  have h14 := hPv ⟨14, by decide⟩
  have e0 := (val_lhs_digit_two (M := nfFold32L 0) h0 h8 (offNat32_lt 0) (by decide) (by decide)).1
  have e1 := (val_lhs_digit (M := nfFold32L 1) h1 h9 h8 (offNat32_lt 1) (by decide) (by decide)).1
  have e2 := (val_lhs_digit (M := nfFold32L 2) h2 h10 h9 (offNat32_lt 2) (by decide) (by decide)).1
  have e3 := (val_lhs_digit (M := nfFold32L 3) h3 h11 h10 (offNat32_lt 3) (by decide) (by decide)).1
  have e4 := (val_lhs_digit (M := nfFold32L 4) h4 h12 h11 (offNat32_lt 4) (by decide) (by decide)).1
  have e5 := (val_lhs_digit (M := nfFold32L 5) h5 h13 h12 (offNat32_lt 5) (by decide) (by decide)).1
  have e6 := (val_lhs_digit (M := nfFold32L 6) h6 h14 h13 (offNat32_lt 6) (by decide) (by decide)).1
  have e7 := (val_lhs_digit_top (M := nfFold32L 7) h7 h14 (offNat32_lt 7) (by decide) (by decide)).1
  rw [polyValue_eight]
...
lemma foldRhs32_bound (env : Environment (F circomPrime))
    ...
    (hq : (Expression.eval env q).val < 2 ^ 39)
    (ht : ∀ i : Fin numLimbs, (Expression.eval env (t[i.val]'i.isLt)).val < 3 * 2 ^ 64) :
    ∀ k : Fin 8, (Expression.eval env (foldRhs32 q t)[k.val]).val < (2 ^ 71 + 3 * 2 ^ 64) := by
  intro k
  ...
  · have hd := val_rhs_digit (k := 0) hq t0
    show (Expression.eval env (foldRhs32 q t)[0]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 1) hq
    show (Expression.eval env (foldRhs32 q t)[1]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 2) hq t1
    show (Expression.eval env (foldRhs32 q t)[2]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 3) hq
    show (Expression.eval env (foldRhs32 q t)[3]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 4) hq t2
    show (Expression.eval env (foldRhs32 q t)[4]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 5) hq
    show (Expression.eval env (foldRhs32 q t)[5]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 6) hq t3
    show (Expression.eval env (foldRhs32 q t)[6]).val < (2 ^ 71 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 7) hq
    show (Expression.eval env (foldRhs32 q t)[7]).val < (2 ^ 71 + 3 * 2 ^ 64)
...
lemma convNat_lt {Ca Cb : ℕ} (env : Environment (F circomPrime))
    ...
    (ha : ∀ i : Fin 8, (Expression.eval env (a[i.val]'i.isLt)).val < Ca)
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb)
    (k : ℕ) : convNat env a b k < 8 * (Ca * Cb) := by
  ...
  have hsum : convNat env a b k ≤ 8 * (Ca * Cb - 1) := by
    unfold convNat
    calc ∑ i : Fin 8, _ ≤ ∑ _i : Fin 8, (Ca * Cb - 1) :=
          Finset.sum_le_sum fun i _ => hterm i
      _ = 8 * (Ca * Cb - 1) := by
          rw [Finset.sum_const, Finset.card_univ, Fintype.card_fin, smul_eq_mul]
  have hpos : 0 < Ca * Cb := Nat.mul_pos hCa hCb
  have : 8 * (Ca * Cb - 1) < 8 * (Ca * Cb) := by omega
  omega

/-- Position-aware bound on a convolution coefficient: the `k`-th cell has only
`triCnt k` terms, not eight. -/
lemma convNat_lt_tri {Ca Cb : ℕ} (env : Environment (F circomPrime))
    (a b : Var (BigInt 8) (F circomPrime))
    (ha : ∀ i : Fin 8, (Expression.eval env (a[i.val]'i.isLt)).val < Ca)
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb)
    (hcap : Ca * Cb ≤ 2 ^ 66) (k : Fin 15) : convNat env a b k.val < triNf k.val := by
  have hCa : 0 < Ca := lt_of_le_of_lt (Nat.zero_le _) (ha ⟨0, by decide⟩)
  have hCb : 0 < Cb := lt_of_le_of_lt (Nat.zero_le _) (hb ⟨0, by decide⟩)
  have hpos : 0 < Ca * Cb := Nat.mul_pos hCa hCb
  have hcnt : 0 < triCnt k.val := by
    have : ∀ j : Fin 15, 0 < triCnt j.val := by decide
    exact this k
  have hterm : ∀ i : Fin 8, (if h : i.val ≤ k.val ∧ k.val - i.val < 8 then
      (Expression.eval env (a[i.val]'i.isLt)).val
        * (Expression.eval env (b[k.val - i.val]'h.2)).val else 0)
      ≤ (if i.val ≤ k.val ∧ k.val - i.val < 8 then 1 else 0) * (Ca * Cb - 1) := by
    intro i
    by_cases h : i.val ≤ k.val ∧ k.val - i.val < 8
    · simp only [dif_pos h, if_pos h, one_mul]
      have hai : (Expression.eval env (a[i.val]'i.isLt)).val < Ca := ha i
      have hbk : (Expression.eval env (b[k.val - i.val]'h.2)).val < Cb := hb ⟨k.val - i.val, h.2⟩
      have := Nat.mul_le_mul (Nat.le_sub_one_of_lt hai) (Nat.le_sub_one_of_lt hbk)
      have hexp : Ca * Cb = (Ca - 1) * (Cb - 1) + (Ca - 1) + (Cb - 1) + 1 := by
        cases Ca with
        | zero => omega
        | succ n =>
          ...
          | succ m => simp only [Nat.succ_sub_one]; ring
      omega
    · simp only [dif_neg h, if_neg h]; omega
  have hsum : convNat env a b k.val ≤ triCnt k.val * (Ca * Cb - 1) := by
    unfold convNat
    calc ∑ i : Fin 8, _ ≤ ∑ i : Fin 8,
          (if i.val ≤ k.val ∧ k.val - i.val < 8 then 1 else 0) * (Ca * Cb - 1) :=
          Finset.sum_le_sum fun i _ => hterm i
      _ = triCnt k.val * (Ca * Cb - 1) := by rw [← Finset.sum_mul]; rfl
  have hstep : triCnt k.val * (Ca * Cb - 1) < triCnt k.val * (Ca * Cb) :=
    Nat.mul_lt_mul_of_pos_left (by omega) hcnt
  have hfin : triCnt k.val * (Ca * Cb) ≤ triCnt k.val * 2 ^ 66 :=
...
/-- The **top** convolution cell is a single product.

A convolution's term count is triangular — `1, 2, …, 8, …, 2, 1` — so the generic
`8 · (Ca·Cb)` bound over-counts at the extremes.  Cell `14` collects only the pair
`(i, k - i) = (7, 7)`, so it is below `Ca·Cb` rather than `8·(Ca·Cb)`.  Since the
quotient width is driven by the top of the folded integer, this is exactly where
the over-count bites. -/
lemma convNat_top_lt {Ca Cb : ℕ} (env : Environment (F circomPrime))
    (a b : Var (BigInt 8) (F circomPrime))
    (ha : ∀ i : Fin 8, (Expression.eval env (a[i.val]'i.isLt)).val < Ca)
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb) :
    convNat env a b 14 < Ca * Cb := by
  have h7a := ha ⟨7, by decide⟩
  have h7b := hb ⟨7, by decide⟩
  have hsplit : convNat env a b 14
      = (Expression.eval env (a[7]'(by decide))).val
        * (Expression.eval env (b[7]'(by decide))).val := by
    simp [convNat, Fin.sum_univ_eight]
  rw [hsplit]
  exact Nat.mul_lt_mul_of_lt_of_le h7a (Nat.le_of_lt h7b)
    (lt_of_le_of_lt (Nat.zero_le _) h7b)
...
/-- `D < 9·2^288 + 2^266` whenever the product coefficients are capped. -/
lemma foldDNat_lt {Ca Cb : ℕ} (env : Environment (F circomPrime))
    (a b : Var (BigInt 8) (F circomPrime))
    (ha : ∀ i : Fin 8, (Expression.eval env (a[i.val]'i.isLt)).val < Ca)
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb)
    (hcap : 8 * (Ca * Cb) ≤ 2 ^ 69) :
    foldDNat env a b < 9 * 2 ^ 290 + 2 ^ 269 := by
  have hctop := convNat_top_lt env a b ha hb
  have hc0 := convNat_lt env a b ha hb 0
  have hc1 := convNat_lt env a b ha hb 1
  have hc2 := convNat_lt env a b ha hb 2
  have hc3 := convNat_lt env a b ha hb 3
  have hc4 := convNat_lt env a b ha hb 4
  have hc5 := convNat_lt env a b ha hb 5
  have hc6 := convNat_lt env a b ha hb 6
  have hc7 := convNat_lt env a b ha hb 7
  have hc8 := convNat_lt env a b ha hb 8
  have hc9 := convNat_lt env a b ha hb 9
  have hc10 := convNat_lt env a b ha hb 10
  have hc11 := convNat_lt env a b ha hb 11
  have hc12 := convNat_lt env a b ha hb 12
  have hc13 := convNat_lt env a b ha hb 13
  have hc14 := convNat_lt env a b ha hb 14
...
def main (input : Var Inputs (F circomPrime)) : Circuit (F circomPrime) Unit := do
  ...
  RangeCheck.circuit qBitsFold32 (by decide) (by decide) q
  let Pc ← interpolatedMul input.a input.b
  GroupedEqXV.circuit 32 gfFold32 posOfFold32 3 vFold32L vFold32R hgvFold32 (by norm_num)
    { lhs := foldLhs32 Pc, rhs := foldRhs32 q input.target }

instance elaborated : ElaboratedCircuit (F circomPrime) Inputs unit main where
  -- q (1) + range check (37) + interpolated product (15) + one carry (44)
  localLength _ := 97
  localLength_eq := by
    intro input offset
    simp only [main, interpolatedMul, circuit_norm, RangeCheck.circuit,
      GroupedEqXV.circuit, GroupedEqXV.elaborated, vFold32L, wfFold32,
      GroupedEqXV.widthAllocFrom, qBitsFold32]
  subcircuitsConsistent := by
    intro input offset
    simp +arith only [main, interpolatedMul, circuit_norm, RangeCheck.circuit,
...
theorem soundness (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 69) :
  ...
  have hbridge := interpolatedMul_eval_bridge env (i₀ + 1 + (qBitsFold32 - 1))
    input_var_a input_var_b hpm
    (interpolatedMul_soundness (i₀ + 1 + (qBitsFold32 - 1)) input_var_a input_var_b env hAB_ops)
  have ha_lt' : ∀ i : Fin 8,
  ...
  have hPv : ∀ k : Fin 15,
      (Expression.eval env (interpolatedMul input_var_a input_var_b
        (i₀ + 1 + (qBitsFold32 - 1))).1[k.val]).val < triNf k.val := by
    intro k
  ...
  have hqv : (Expression.eval env (var (F := F circomPrime) { index := i₀ })).val < 2 ^ 39 := by
    have h36 : (Expression.eval env (var (F := F circomPrime) { index := i₀ })).val < 2 ^ 38 :=
      hq_lt
...
theorem completeness (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 69) :
  ...
  have hNbp : 8 * (Ca * Cb) < circomPrime := by
    have h2 : (2 : ℕ) ^ 69 < circomPrime := by decide
    omega
  have h_pvAB := interpolatedMul_usesLocalWitnesses (i₀ + 1 + (qBitsFold32 - 1))
    (i₀ + 1 + (qBitsFold32 - 1)) input_var_a input_var_b env rfl hAB_uses
  have hbridge := interpolatedMul_eval_bridge_uses env.toEnvironment
    (i₀ + 1 + (qBitsFold32 - 1)) input_var_a input_var_b h_pvAB
  have hcap64 : Ca * Cb ≤ 2 ^ 66 := by omega
  have hPv : ∀ k : Fin 15,
      (Expression.eval env.toEnvironment (interpolatedMul input_var_a input_var_b
        (i₀ + 1 + (qBitsFold32 - 1))).1[k.val]).val < triNf k.val := by
    intro k
    rw [hbridge k,
  ...
  have hDlt : D < 9 * 2 ^ 290 + 2 ^ 269 := foldDNat_lt env.toEnvironment input_var_a input_var_b
    ha_lt' hb_lt' hcap
  ...
  have hqvar : (Expression.eval env.toEnvironment
      (var (F := F circomPrime) { index := i₀ })).val < 2 ^ 39 := by
    show (env.get i₀).val < 2 ^ 39
    rw [hqval]
    have : (2 : ℕ) ^ 38 ≤ 2 ^ 39 := by norm_num
    omega
  refine ⟨by rw [hqval]; exact hqlt,
    interpolatedMul_completeness (i₀ + 1 + (qBitsFold32 - 1)) input_var_a input_var_b env h_pvAB,
...
def circuit (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 69) :
...
lemma convNat_stable (a b : Var (BigInt 8) (F circomPrime))
...
lemma qNat_stable (input : Var Inputs (F circomPrime))
  ...
  rw [convNat_stable input.a input.b ha hb 0,
    convNat_stable input.a input.b ha hb 1,
    convNat_stable input.a input.b ha hb 2,
    convNat_stable input.a input.b ha hb 3,
    convNat_stable input.a input.b ha hb 4,
    convNat_stable input.a input.b ha hb 5,
    convNat_stable input.a input.b ha hb 6,
    convNat_stable input.a input.b ha hb 7,
    convNat_stable input.a input.b ha hb 8,
    convNat_stable input.a input.b ha hb 9,
    convNat_stable input.a input.b ha hb 10,
    convNat_stable input.a input.b ha hb 11,
    convNat_stable input.a input.b ha hb 12,
    convNat_stable input.a input.b ha hb 13,
    convNat_stable input.a input.b ha hb 14,
    bigInt_map_eval_eq_of_eval_eq ht]

attribute [local irreducible] interpolatedMul RangeCheck.circuit GroupedEqXV.circuit

theorem computableWitnesses (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 69) :
  ...
  · exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      ...
      (by
        ...
        have hPc_i : ∀ (i : ℕ) (hi : i < 15),
          ...
          intro i hi
          have := congrArg (fun v : Vector (F circomPrime) (2 * 8 - 1) => v[i]'hi) hPc
        ...
        have hL := foldLhs32_stable (E1 := e1.toEnvironment) (E2 := e2.toEnvironment) pcv hPc_i
        have hR := foldRhs32_stable (E1 := e1.toEnvironment) (E2 := e2.toEnvironment)
          qv input.target (by rw [h_qvar]; exact hq) ht_i
        simp only [circuit_norm]
        rw [hL, hR])
      (GroupedEqXV.computableWitnesses 32 gfFold32 posOfFold32 3 vFold32L vFold32R hgvFold32
        (by norm_num))
      env env'
