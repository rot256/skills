import Solution.Secp256k1ScalarMul.ParamsFold32T
...
lemma val_rhs_digit {q t : F circomPrime} {k : ℕ}
    (hq : q.val < 2 ^ 37) (ht : t.val < 3 * 2 ^ 64) :
    (q * ((pNat32 k : ℕ) : F circomPrime) + t).val = q.val * pNat32 k + t.val ∧
      q.val * pNat32 k + t.val < (2 ^ 69 + 3 * 2 ^ 64) := by
  have hpk := pNat32_lt k
  have hmulb : q.val * pNat32 k < 2 ^ 69 := by
    calc q.val * pNat32 k < 2 ^ 37 * 2 ^ 32 :=
          Nat.mul_lt_mul_of_lt_of_le hq (by omega) (by positivity)
      _ = 2 ^ 69 := by norm_num
  have hsum : q.val * pNat32 k + t.val < (2 ^ 69 + 3 * 2 ^ 64) := by
    have h1 : (3 : ℕ) * 2 ^ 64 + 2 ^ 69 ≤ (2 ^ 69 + 3 * 2 ^ 64) := by decide
    omega
...
lemma val_rhs_digit_odd {q : F circomPrime} {k : ℕ} (hq : q.val < 2 ^ 37) :
    (q * ((pNat32 k : ℕ) : F circomPrime)).val = q.val * pNat32 k ∧
      q.val * pNat32 k < (2 ^ 69 + 3 * 2 ^ 64) := by
  have hpk := pNat32_lt k
  have hmulb : q.val * pNat32 k < 2 ^ 69 := by
    calc q.val * pNat32 k < 2 ^ 37 * 2 ^ 32 :=
          Nat.mul_lt_mul_of_lt_of_le hq (by omega) (by positivity)
...
lemma foldLhs32_bound (env : Environment (F circomPrime))
    ...
    ∀ k : Fin 8, (Expression.eval env (foldLhs32 Pv)[k.val]).val < nfFold32TL k.val := by
  intro k
  ...
  fin_cases k
  · have hd := val_lhs_digit_two (M := nfFold32TL 0) h0 h8 (offNat32_lt 0) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[0]).val < nfFold32TL 0
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 1) h1 h9 h8 (offNat32_lt 1) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[1]).val < nfFold32TL 1
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 2) h2 h10 h9 (offNat32_lt 2) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[2]).val < nfFold32TL 2
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 3) h3 h11 h10 (offNat32_lt 3) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[3]).val < nfFold32TL 3
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 4) h4 h12 h11 (offNat32_lt 4) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[4]).val < nfFold32TL 4
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 5) h5 h13 h12 (offNat32_lt 5) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[5]).val < nfFold32TL 5
    ...
    exact hd.2
  · have hd := val_lhs_digit (M := nfFold32TL 6) h6 h14 h13 (offNat32_lt 6) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[6]).val < nfFold32TL 6
    ...
    exact hd.2
  · have hd := val_lhs_digit_top (M := nfFold32TL 7) h7 h14 (offNat32_lt 7) (by decide) (by decide)
    show (Expression.eval env (foldLhs32 Pv)[7]).val < nfFold32TL 7
...
lemma foldLhs32_polyValue (env : Environment (F circomPrime))
  ...
  have h13 := hPv ⟨13, by decide⟩
  have h14 := hPv ⟨14, by decide⟩
  have e0 := (val_lhs_digit_two (M := nfFold32TL 0) h0 h8 (offNat32_lt 0) (by decide) (by decide)).1
  have e1 := (val_lhs_digit (M := nfFold32TL 1) h1 h9 h8 (offNat32_lt 1) (by decide) (by decide)).1
  have e2 := (val_lhs_digit (M := nfFold32TL 2) h2 h10 h9 (offNat32_lt 2) (by decide) (by decide)).1
  have e3 := (val_lhs_digit (M := nfFold32TL 3) h3 h11 h10 (offNat32_lt 3) (by decide) (by decide)).1
  have e4 := (val_lhs_digit (M := nfFold32TL 4) h4 h12 h11 (offNat32_lt 4) (by decide) (by decide)).1
  have e5 := (val_lhs_digit (M := nfFold32TL 5) h5 h13 h12 (offNat32_lt 5) (by decide) (by decide)).1
  have e6 := (val_lhs_digit (M := nfFold32TL 6) h6 h14 h13 (offNat32_lt 6) (by decide) (by decide)).1
  have e7 := (val_lhs_digit_top (M := nfFold32TL 7) h7 h14 (offNat32_lt 7) (by decide) (by decide)).1
  rw [polyValue_eight]
...
lemma foldRhs32_bound (env : Environment (F circomPrime))
    ...
    (hq : (Expression.eval env q).val < 2 ^ 37)
    (ht : ∀ i : Fin numLimbs, (Expression.eval env (t[i.val]'i.isLt)).val < 3 * 2 ^ 64) :
    ∀ k : Fin 8, (Expression.eval env (foldRhs32 q t)[k.val]).val < (2 ^ 69 + 3 * 2 ^ 64) := by
  intro k
  ...
  · have hd := val_rhs_digit (k := 0) hq t0
    show (Expression.eval env (foldRhs32 q t)[0]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 1) hq
    show (Expression.eval env (foldRhs32 q t)[1]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 2) hq t1
    show (Expression.eval env (foldRhs32 q t)[2]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 3) hq
    show (Expression.eval env (foldRhs32 q t)[3]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 4) hq t2
    show (Expression.eval env (foldRhs32 q t)[4]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 5) hq
    show (Expression.eval env (foldRhs32 q t)[5]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit (k := 6) hq t3
    show (Expression.eval env (foldRhs32 q t)[6]).val < (2 ^ 69 + 3 * 2 ^ 64)
  ...
  · have hd := val_rhs_digit_odd (k := 7) hq
    show (Expression.eval env (foldRhs32 q t)[7]).val < (2 ^ 69 + 3 * 2 ^ 64)
...
lemma convNat_lt_tri {Ca Cb : ℕ} (env : Environment (F circomPrime))
    ...
    (ha : ∀ i : Fin 8, (Expression.eval env (a[i.val]'i.isLt)).val < Ca)
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb)
    (hcap : Ca * Cb ≤ 2 ^ 64) (k : Fin 15) : convNat env a b k.val < triNf k.val := by
  have hCa : 0 < Ca := lt_of_le_of_lt (Nat.zero_le _) (ha ⟨0, by decide⟩)
  have hCb : 0 < Cb := lt_of_le_of_lt (Nat.zero_le _) (hb ⟨0, by decide⟩)
  have hpos : 0 < Ca * Cb := Nat.mul_pos hCa hCb
  ...
  have hfin : triCnt k.val * (Ca * Cb) ≤ triCnt k.val * 2 ^ 64 :=
...
lemma foldDNat_lt {Ca Cb : ℕ} (env : Environment (F circomPrime))
    ...
    (hb : ∀ i : Fin 8, (Expression.eval env (b[i.val]'i.isLt)).val < Cb)
    (hcap : 8 * (Ca * Cb) ≤ 2 ^ 67) :
    foldDNat env a b < 9 * 2 ^ 288 + 2 ^ 266 := by
...
instance elaborated : ElaboratedCircuit (F circomPrime) Inputs unit main where
  -- q (1) + range check (35) + interpolated product (15) + one carry (42)
  localLength _ := 93
  localLength_eq := by
    intro input offset
...
theorem soundness (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 67) :
  ...
  have hbridge := interpolatedMul_eval_bridge env (i₀ + 1 + (qBitsFold32T - 1))
    input_var_a input_var_b hpm
    (interpolatedMul_soundness (i₀ + 1 + (qBitsFold32T - 1)) input_var_a input_var_b env hAB_ops)
  have ha_lt' : ∀ i : Fin 8,
  ...
  have hPv : ∀ k : Fin 15,
      (Expression.eval env (interpolatedMul input_var_a input_var_b
        (i₀ + 1 + (qBitsFold32T - 1))).1[k.val]).val < triNf k.val := by
    intro k
...
theorem completeness (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 67) :
  ...
  have hNbp : 8 * (Ca * Cb) < circomPrime := by
    have h2 : (2 : ℕ) ^ 69 < circomPrime := by decide
    omega
  have h_pvAB := interpolatedMul_usesLocalWitnesses (i₀ + 1 + (qBitsFold32T - 1))
    (i₀ + 1 + (qBitsFold32T - 1)) input_var_a input_var_b env rfl hAB_uses
  have hbridge := interpolatedMul_eval_bridge_uses env.toEnvironment
    (i₀ + 1 + (qBitsFold32T - 1)) input_var_a input_var_b h_pvAB
  have hcap64 : Ca * Cb ≤ 2 ^ 64 := by omega
  have hPv : ∀ k : Fin 15,
  ...
  refine ⟨by rw [hqval]; exact hqlt,
    interpolatedMul_completeness (i₀ + 1 + (qBitsFold32T - 1)) input_var_a input_var_b env h_pvAB,
