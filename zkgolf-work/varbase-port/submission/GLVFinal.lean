...
lemma jacobian_phiPoint_eq :
    Bridge.mkPoint JacobianCert.PhiG JacobianCert.PhiG_onCurve =
      Phi.hom ProofBlocker.gPoint := by
  rfl
...
theorem point_card_eq_order : Nat.card Bridge.W.Point = order :=
  ProofBlocker.point_card_eq_order_of_order_nsmul order_nsmul_gPoint
