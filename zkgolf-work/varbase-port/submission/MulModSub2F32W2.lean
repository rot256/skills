...
/-!
# `r = a·b − s1 − s2 (mod p)` with *widened* operands in 32-bit limbs
...
given as eight 32-bit limbs, so the certificate is `MulModFold32` (99/101)
instead of `MulModFold` (176/178).

Cost: 4 + 260/265 + 99/101 = 363/366, against `MulModSub2`'s 440/445.
Callers get the 32-bit view of a witnessed slope for free out of `DivOrZeroF3`.
-/

namespace Solution.Secp256k1ScalarMul
namespace MulModSub2F32W2
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  MulModFold32.circuitInv widen33 widen33 hcap33
    { a := a, b := b,
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  circuit_proof_start [ValidP.circuit, ValidP.main, ValidP.Assumptions, ValidP.Spec,
    MulModFold32.circuitInv, MulModFold32.Assumptions, MulModFold32.Spec]
  ...
  · -- conclude the spec from the congruence
    ...
    rw [decodeFe_emuOf32V_33 ha_valid, decodeFe_emuOf32V_33 hb_valid]
    simp only [decodeFe]
...
theorem computableWitnesses : circuit.base.ComputableWitnesses := by
  ...
  · -- MulModFold { a, b, r + s1 + s2 }
    exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      (Parent := Inputs)
      (MulModFold32.circuitInv widen33 widen33 hcap33) _
      { a := a, b := b,
        target := Vector.ofFn fun k : Fin numLimbs =>
          r[k.val]'k.isLt + s1[k.val]'k.isLt + s2[k.val]'k.isLt }
      _
      (by
        ...
        rw [MulModFold32.Inputs.mk.injEq]
        refine ⟨ha, hb, ?_⟩
        apply Vector.ext
        intro i hi
      ...
      (MulModFold32.computableWitnessesInv widen33 widen33 hcap33) env env'
