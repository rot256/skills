...
lemma split3_sum {t x : Specs.Secp256k1.Fp} (h : t = x + x + x) :
    ∃ g : Emu (F circomPrime), decodeFe g = x + x ∧ t = decodeFe g + x :=
  ⟨emuOfNat ((x + x).val), decodeFe_emuOfNat_val _,
...
lemma threePLimbs_limb_lt : ∀ j : Fin 7, ((threePLimbs)[j.val]).val < 2 ^ limbBits := by
  intro j
  rw [threePLimbs, Vector.getElem_ofFn, val_limbOfNat]
  exact limbOfNat_lt _ _


lemma polyValue_threePLimbs : polyValue limbBits threePLimbs = 3 * P256 := by
  rw [polyValue]
  have hsum : (∑ k : Fin 7, ((threePLimbs)[k.val]).val * 2 ^ (limbBits * k.val))
      = ∑ k ∈ Finset.range 7,
          (3 * P256 / 2 ^ (limbBits * k) % 2 ^ limbBits) * 2 ^ (limbBits * k) := by
    rw [← Fin.sum_univ_eq_sum_range
      (fun k => (3 * P256 / 2 ^ (limbBits * k) % 2 ^ limbBits) * 2 ^ (limbBits * k))]
    refine Finset.sum_congr rfl fun i _ => ?_
    rw [threePLimbs, Vector.getElem_ofFn, val_limbOfNat]
    rfl
  rw [hsum, limb_decomp_mod,
    Nat.mod_eq_of_lt (by decide : 3 * P256 < 2 ^ (limbBits * 7))]
...
lemma decodeFe_sum3 (u : Emu (F circomPrime)) (hu : u.Normalized limbBits) :
    decodeFe (Vector.ofFn fun k : Fin numLimbs =>
        u[k.val]'k.isLt + u[k.val]'k.isLt + u[k.val]'k.isLt)
      = decodeFe u + decodeFe u + decodeFe u := by
  show ((BigInt.value limbBits (Vector.ofFn fun k : Fin numLimbs =>
      u[k.val]'k.isLt + u[k.val]'k.isLt + u[k.val]'k.isLt) : ℕ) : Specs.Secp256k1.Fp) = _
  rw [value_sum3 u u u hu hu hu]
  push_cast
  rfl


lemma limb_sum3_bound (u : Emu (F circomPrime)) (hu : u.Normalized limbBits)
    (i : Fin numLimbs) :
    ((Vector.ofFn fun k : Fin numLimbs =>
        u[k.val]'k.isLt + u[k.val]'k.isLt + u[k.val]'k.isLt)[i.val]'i.isLt).val
      < 3 * 2 ^ limbBits := by
  rw [Vector.getElem_ofFn]
  have h : (u[i.val]'i.isLt).val < 2 ^ limbBits := hu i
  exact limb_sum3_lt h h h


lemma value_sum3_bound (u : Emu (F circomPrime)) (hu : Fe.Valid u) :
    BigInt.value limbBits (Vector.ofFn fun k : Fin numLimbs =>
        u[k.val]'k.isLt + u[k.val]'k.isLt + u[k.val]'k.isLt) < 3 * P256 := by
  rw [value_sum3 u u u hu.1 hu.1 hu.1]
  have := hu.2
  omega
...
lemma map_eval_sum3_cells (env : Environment (F circomPrime)) (f : ℕ → ℕ) :
    Vector.map (Expression.eval env) (Vector.ofFn fun k : Fin numLimbs =>
        var (F := F circomPrime) { index := f k.val }
          + var { index := f k.val } + var { index := f k.val })
      = Vector.ofFn (fun k : Fin numLimbs =>
        (Vector.map (Expression.eval env)
          (Vector.mapRange numLimbs fun i => var { index := f i }))[k.val]
          + (Vector.map (Expression.eval env)
            (Vector.mapRange numLimbs fun i => var { index := f i }))[k.val]
          + (Vector.map (Expression.eval env)
            (Vector.mapRange numLimbs fun i => var { index := f i }))[k.val]) := by
  apply Vector.ext
  intro j hj
...
lemma limb_borrow_bound (qy py : Emu (F circomPrime))
    (hqy : qy.Normalized limbBits) (hpy : py.Normalized limbBits) (i : Fin numLimbs) :
    ((Vector.ofFn fun k : Fin numLimbs =>
        qy[k.val]'k.isLt + (((twoPBorrowDigit k.val : ℕ) : F circomPrime)
          - py[k.val]'k.isLt))[i.val]'i.isLt).val < 3 * 2 ^ limbBits := by
  rw [Vector.getElem_ofFn]
  exact limb_borrow_lt i.val (hqy i) (hpy i)
...
lemma value_sum2_zero_iff (u : Emu (F circomPrime)) (hu : Fe.Valid u) :
    BigInt.value limbBits (Vector.ofFn fun k : Fin numLimbs =>
        u[k.val]'k.isLt + u[k.val]'k.isLt) = 0
      ↔ decodeFe (Vector.ofFn fun k : Fin numLimbs =>
          u[k.val]'k.isLt + u[k.val]'k.isLt) = 0 := by
  constructor
  · intro h
    show ((BigInt.value limbBits _ : ℕ) : Specs.Secp256k1.Fp) = 0
    rw [h, Nat.cast_zero]
  · intro h
    apply value_sum2_alias u hu
    have hdvd : (P256 : ℕ) ∣ BigInt.value limbBits (Vector.ofFn fun k : Fin numLimbs =>
        u[k.val]'k.isLt + u[k.val]'k.isLt) :=
