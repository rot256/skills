...
def betaConst : Var Emu (F circomPrime) := emuConst betaNat
...
def tablePoint (input : Inputs (F circomPrime)) (t : ℕ) : GroupPoint Fp :=
  subsetPoint (signedBase input) t
...
lemma beta_valid : Fe.Valid (emuOfNat betaNat) := by
  exact DivOrZero.fe_valid_emuOfNat (by norm_num [betaNat, P256, p])
...
def evalFP (env : Environment (F circomPrime))
    (P : Var FlaggedPoint (F circomPrime)) : FlaggedPoint (F circomPrime) :=
  { x := Vector.map (Expression.eval env) P.x
    y := Vector.map (Expression.eval env) P.y
    isInf := Expression.eval env P.isInf }

def evalRaw (env : Environment (F circomPrime))
    (input : Var RawTable (F circomPrime)) : RawTable (F circomPrime) :=
  RawTable.mk (evalFP env input.t0) (evalFP env input.t1)
    (evalFP env input.t2) (evalFP env input.t3)
    (evalFP env input.t4) (evalFP env input.t5)
    (evalFP env input.t6) (evalFP env input.t7)
    (evalFP env input.t8) (evalFP env input.t9)
    (evalFP env input.t10) (evalFP env input.t11)
    (evalFP env input.t12) (evalFP env input.t13)
    (evalFP env input.t14) (evalFP env input.t15)

set_option maxRecDepth 8192 in
lemma eval_pack_entry (env : Environment (F circomPrime))
    (input : Var RawTable (F circomPrime)) (i : Fin 16) :
    entry
      { tx := eval env (pack input).tx
        ty := eval env (pack input).ty
        tinf := Vector.map (Expression.eval env) (pack input).tinf }
      i.val i.isLt = rawEntry (evalRaw env input) i := by
  fin_cases i <;> simp [pack, evalRaw, evalFP, entry, rawEntry, circuit_norm,
    ← getElem_eval_vector]
...
set_option maxRecDepth 8192 in
lemma eval_pack_entry_raw (env : Environment (F circomPrime))
    (input : Var RawTable (F circomPrime)) (i : Fin 16) :
    entry (eval env (pack input)) i.val i.isLt = rawEntry (evalRaw env input) i := by
  fin_cases i <;> simp [pack, evalRaw, evalFP, entry, rawEntry, circuit_norm,
    ← getElem_eval_vector]


lemma rawEntry_evalRaw_eq_eval (env : Environment (F circomPrime))
    (input : Var RawTable (F circomPrime)) (i : Fin 16) :
    rawEntry (evalRaw env input) i = rawEntry (eval env input) i := by
  calc
    rawEntry (evalRaw env input) i =
        entry (eval env (pack input)) i.val i.isLt :=
      (eval_pack_entry_raw env input i).symm
    _ = rawEntry (eval env input) i := eval_pack_entry_whole env input i
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  let raw ← subcircuit Subset.circuit bases
  return Pack.pack raw

def basesAt (input : Var Inputs (F circomPrime)) (offset : ℕ) :
    Var Bases (F circomPrime) :=
  Prepare.circuit.output input offset

def rawAt (input : Var Inputs (F circomPrime)) (offset : ℕ) :
    Var RawTable (F circomPrime) :=
  Subset.circuit.output (basesAt input offset)
    (offset + Prepare.circuit.localLength input)

def tableAt (input : Var Inputs (F circomPrime)) (offset : ℕ) :
    Var Table (F circomPrime) :=
  Pack.pack (rawAt input offset)

def totalLength (_input : Var Inputs (F circomPrime)) : ℕ := 14325

instance elaborated : ElaboratedCircuit (F circomPrime) Inputs Table main where
  localLength := totalLength
  localLength_eq input offset := by
    simp only [main, totalLength, circuit_norm, Prepare.localLength,
      Subset.localLength]
  output := tableAt
  output_eq input offset := by
    simp only [main, tableAt, rawAt, basesAt, circuit_norm]
...
def Spec (input : Inputs (F circomPrime)) (out : Table (F circomPrime)) : Prop :=
  ∀ i : Fin 16, (entry out i.val i.isLt).Valid ∧
    decodePoint (entry out i.val i.isLt) = tablePoint input i.val ∧
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  ...
  obtain ⟨hp0, hs0⟩ := h_holds
  have hp0' := hp0 (by
    simpa only [circuit_norm] using h_assumptions.1)
  have hp' : Prepare.Spec (eval env input_var)
      (eval env (basesAt input_var i₀)) := by
    simpa only [circuit_norm] using hp0'
  have hsubAss : Subset.Assumptions (eval env (basesAt input_var i₀)) :=
  ...
  have hs0' := hs0 (by
    simpa only [circuit_norm] using hsubAss)
  have hs' : Subset.Spec (eval env (basesAt input_var i₀))
      (eval env (rawAt input_var i₀)) := by
    simpa only [circuit_norm] using hs0'
  have hbase (j : Fin 4) := (hp'.1 j).2
  dsimp only [elaborated, ElaboratedCircuit.output, Spec, tableAt]
  constructor
  · intro i
    rw [Pack.eval_pack_entry_whole]
    have hi := hs' i
    have hbaseFn := funext hbase
    rw [hbaseFn] at hi
    simpa only [tablePoint] using hi
  · simp only [main, circuit_norm]
    constructor
    · exact Or.inr (by simpa only [circuit_norm] using h_assumptions.1)
    · exact Or.inr (by simpa only [circuit_norm] using hsubAss)

set_option maxRecDepth 65536 in
set_option maxHeartbeats 4000000 in
theorem completeness : Completeness (F circomPrime) main Assumptions := by
  circuit_proof_start [Prepare.circuit, Prepare.Assumptions, Prepare.Spec,
    Subset.circuit, Subset.Assumptions, Subset.Spec]
  obtain ⟨hp, -⟩ := h_env
  have hp' := hp h_assumptions.1
  exact ⟨h_assumptions.1, subsetAssumptions_of_prepare_spec hp'
    h_assumptions.2.1 h_assumptions.2.2.1
    h_assumptions.1.2.1 h_assumptions.2.2.2⟩
