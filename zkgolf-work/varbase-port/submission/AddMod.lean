...
def main (input : Var Inputs (F circomPrime)) :
  ...
  -- A 2-operand add/sub has only `numLimbs` nonzero coefficients (plus one
  -- carry position), so the equality certificate uses the width-5 vector, and
  -- since every coefficient is additive-scale (`< 2^65 + 2`) the narrow-carry
  -- `EqViaCarriesN` (`eqNParamsAdd`) runs each offset carry at 3 bits instead
  -- of the multiplication width 69.
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  -- witness r = (a + b + c) % P256, the quotient q = (a + b + c) / P256 ∈ {0,1,2},
  -- and the auxiliary t = q·(q−1)
  let r ← ProvableType.witness (α := Emu) fun env =>
    emuOfNat ((evalEmu env a + evalEmu env b + evalEmu env c) % P256)
  let q ← ProvableType.witness (α := field) fun env =>
    (((evalEmu env a + evalEmu env b + evalEmu env c) / P256 : ℕ) : F circomPrime)
  let t ← ProvableType.witness (α := field) fun env =>
    (((evalEmu env a + evalEmu env b + evalEmu env c) / P256 : ℕ) : F circomPrime)
      * ((((evalEmu env a + evalEmu env b + evalEmu env c) / P256 : ℕ) : F circomPrime) - 1)
  ...
  -- a + b + c = q·P256 + r as integers, limb-coefficient-wise (3-sum scale).
  let lhs : Vector (Expression (F circomPrime)) (2 * 3 - 1) :=
    Vector.mapFinRange (2 * 3 - 1) fun k =>
      if h : k.val < numLimbs then a[k.val]'h + b[k.val]'h + c[k.val]'h else 0
  let rhs : Vector (Expression (F circomPrime)) (2 * 3 - 1) :=
    Vector.mapFinRange (2 * 3 - 1) fun k =>
      if h : k.val < numLimbs then q * pConst[k.val]'h + r[k.val]'h else 0
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  ...
  apply soundness_core3N i₀ env input_var_a input_var_b input_var_c input_a input_b input_c
...
theorem completeness : Completeness (F circomPrime) main Assumptions := by
  ...
  have hcore := completeness_core3N i₀ env.toEnvironment
...
theorem computableWitnesses : circuit.base.ComputableWitnesses := by
  ...
  let rCircuit : Circuit (F circomPrime) (Var Emu (F circomPrime)) :=
    ProvableType.witness (α := Emu) fun env =>
      emuOfNat ((evalEmu env input.a + evalEmu env input.b + evalEmu env input.c) % P256)
  let r := rCircuit.output offset
  let qOffset := offset + rCircuit.localLength offset
  let qCircuit : Circuit (F circomPrime) (Var field (F circomPrime)) :=
    ProvableType.witness (α := field) fun env =>
      (((evalEmu env input.a + evalEmu env input.b + evalEmu env input.c) / P256 : ℕ)
        : F circomPrime)
  let q := qCircuit.output qOffset
  ...
  let lhs : Vector (Expression (F circomPrime)) (2 * 3 - 1) :=
    Vector.mapFinRange (2 * 3 - 1) fun k =>
      if h : k.val < numLimbs then input.a[k.val]'h + input.b[k.val]'h + input.c[k.val]'h else 0
  let rhs : Vector (Expression (F circomPrime)) (2 * 3 - 1) :=
    Vector.mapFinRange (2 * 3 - 1) fun k =>
      if h : k.val < numLimbs then q * pConst[k.val]'h + r[k.val]'h else 0
  have h_r_len : rCircuit.localLength offset = numLimbs := by
            ...
            · rename_i hlt
              ...
              have hr : Expression.eval env.toEnvironment r[i] =
                ...
                have hr_vec := emuWitnessOutput_stable
                  (offset := offset) (k := k)
                  (fun env =>
                    emuOfNat ((evalEmu env input.a + evalEmu env input.b + evalEmu env input.c)
                      % P256))
                  h_agree (by
                    have hbase : offset + numLimbs ≤ validOffset := by
