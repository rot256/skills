...
`MulModFold32M` (163/165) rather than the base-`2^64` `MulModFold` (176/178):
...
relative to the ordinary `MulModFold32M` endpoint.
-/

namespace Solution.Secp256k1ScalarMul
namespace MulModSub2W32M
...
lemma hcapM : 4 * (widen33 * (3 * 2 ^ 64)) ≤ 3 * 2 ^ 99 := by
  simp only [widen33]
  norm_num
...
/-- The value denoted by eight 32-bit limbs, in `Fp`. -/
def decodeFe32 (v : BigInt 8 (F circomPrime)) : Specs.Secp256k1.Fp :=
  ((BigInt.value 32 v : ℕ) : Specs.Secp256k1.Fp)
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  MulModFold32M.circuitInv widen33 (3 * 2 ^ 64) hcapM
    { a := a, b := MulModFold32M.expand32 b,
...
def Assumptions (input : Inputs (F circomPrime)) : Prop :=
  Norm33 input.a ∧ (∀ i : Fin numLimbs, (input.b[i.val]).val < 3 * 2 ^ limbBits) ∧
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  circuit_proof_start [ValidP.circuit, ValidP.main, ValidP.Assumptions, ValidP.Spec,
    MulModFold32M.circuitInv, MulModFold32M.Assumptions, MulModFold32M.Spec]
  ...
  have hbv : BigInt.value 32
        (Vector.map (Expression.eval env) (MulModFold32M.expand32 input_var_b))
      = BigInt.value limbBits (Vector.map (Expression.eval env) input_var_b) := by
    rw [MulModFold32M.eval_expand32, MulModFold32M.value_expand32]
  have hexp_limb : ∀ i : Fin 8,
      (Expression.eval env ((MulModFold32M.expand32 input_var_b)[i.val])).val
        < 3 * 2 ^ 64 := by
    ...
    intro i
    have h := MulModFold32M.expand32V_lt (C := 3 * 2 ^ 64) (by positivity) hb' i
    rw [← MulModFold32M.eval_expand32] at h
    simpa [Vector.getElem_map] using h
  have hspec := hcert ⟨ha_valid, hexp_limb,
    MulModFold32M.eval_expand32_odd_zero env input_var_b, ?_, ?_⟩
  · -- conclude the spec from the congruence
    ...
    rw [decodeFe_emuOf32V_33 ha_valid]
    simp only [decodeFe]
...
theorem completeness : Completeness (F circomPrime) main Assumptions := by
  ...
  have hbv : BigInt.value 32
        (Vector.map (Expression.eval env.toEnvironment) (MulModFold32M.expand32 input_var_b))
      = BigInt.value limbBits (Vector.map (Expression.eval env.toEnvironment) input_var_b) := by
    rw [MulModFold32M.eval_expand32, MulModFold32M.value_expand32]
  have hexp_limb : ∀ i : Fin 8,
      (Expression.eval env.toEnvironment
        ((MulModFold32M.expand32 input_var_b)[i.val])).val < 3 * 2 ^ 64 := by
    have hb' : ∀ j : Fin numLimbs,
  ...
  refine ⟨⟨hr_norm, by rw [hr_val]; exact hr_lt⟩,
    ⟨ha_valid, hexp_limb,
      MulModFold32M.eval_expand32_odd_zero env.toEnvironment input_var_b, ?_, ?_⟩, ?_⟩
  · -- target limbs < 3·2^64
    intro i
    rw [Vector.getElem_ofFn]
...
theorem computableWitnesses : circuit.base.ComputableWitnesses := by
  ...
  · -- MulModFold { a, b, r + s1 + s2 }
    exact Challenge.Utils.ComputableWitnessLemmas.FormalAssertion.assertion_flatStructuralComputableWitnesses_of_condition
      (Parent := Inputs)
      (MulModFold32M.circuitInv widen33 (3 * 2 ^ 64) hcapM) _
      { a := a, b := MulModFold32M.expand32 b,
        target := Vector.ofFn fun k : Fin numLimbs =>
          r[k.val]'k.isLt + s1[k.val]'k.isLt + s2[k.val]'k.isLt }
      _
      (by
        ...
        rw [MulModFold32M.Inputs.mk.injEq]
        refine ⟨ha, ?_, ?_⟩
        · rw [MulModFold32M.eval_expand32, MulModFold32M.eval_expand32]
          exact congrArg MulModFold32M.expand32V hb
        apply Vector.ext
        intro i hi
      ...
      (MulModFold32M.computableWitnessesInv widen33 (3 * 2 ^ 64) hcapM) env env'
