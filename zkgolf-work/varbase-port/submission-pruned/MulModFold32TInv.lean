...
attribute [local irreducible] interpolatedMul RangeCheck.circuit GroupedEqXV.circuitNoTop GroupedEqXV.mainNoTop qInv lhsPolyInv targetPolyInv
...
theorem soundnessInv (Ca Cb : ℕ) (hcap : 8 * (Ca * Cb) ≤ 2 ^ 67) :
  ...
  have hqv : (Expression.eval env
      (qInv (interpolatedMul input_var_a input_var_b i₀).1 input_var_target)).val < 2 ^ 37 := by
    have h36 : (Expression.eval env
        (qInv (interpolatedMul input_var_a input_var_b i₀).1 input_var_target)).val < 2 ^ 36 :=
      hq_lt
