...
instance elaborated (P : BigIntParams p m) [Fact (p > 2)] :
  ...
  -- d : m witnesses; Normalize : m*(B-1); carry bits : m
  localLength _ := m + m * (P.B - 1) + m
  localLength_eq := by
    intro input offset
...
def circuit (P : BigIntParams p m) [Fact (p > 2)] :
    ...
    soundness := by
      ...
      set Cn : ℕ → ℕ := fun k => (env.get (i₀ + m + m * (B - 1) + k)).val with hCn
      -- digit bounds
      ...
      have h_limb : ∀ k : ℕ, (hk : k < m) →
        ...
        have hrhs_lt : (input.rhs[k]'hk).val
            + (env.get (i₀ + m + m * (B - 1) + k)).val * 2 ^ B < p := by
    ...
    completeness := by
      ...
      have hCn_bit : ∀ k : ℕ, k < m → IsBool (env.get (i₀ + m + m * (B - 1) + k)) := by
        intro k hk
      ...
      · -- 3. per-limb field equation
        ...
        have hCk : ((Cn i.val : ℕ) : F p) = env.get (i₀ + m + m * (B - 1) + i.val) := by
        ...
        have hcast_eq : (input.lhs[i.val]'hk) + env.get (i₀ + i.val)
            ...
            = (input.rhs[i.val]'hk) + env.get (i₀ + m + m * (B - 1) + i.val) * (2 ^ B : F p) := by
      ...
      · -- 4. top carry is zero
        ...
        have hval0 : (env.get (i₀ + m + m * (B - 1) + (m - 1))).val = 0 := by
