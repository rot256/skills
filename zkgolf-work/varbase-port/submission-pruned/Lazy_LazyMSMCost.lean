...
/-- `64 · 1604 + 4 + 176 + 176` allocations, `64 · 1617 + 1 + 4 + 178 + 178 + 4` rows. -/
def cost : Count := ⟨103012, 103853⟩

lemma stepBody_cost (input : Var Inputs (F circomPrime)) (acc : Var LazyPt (F circomPrime))
    (k : Fin 64) : CostIs (stepBody input acc k) ⟨1604, 1617⟩ := by
  rw [show (⟨1604, 1617⟩ : Count) = varLookupCost + Step.stepCost from by decide]
  unfold stepBody
...
theorem costIs_main (input : Var Inputs (F circomPrime)) : CostIs (main input) cost := by
  rw [show cost = ⟨64 * 1604, 64 * 1617⟩ + (⟨0, 1⟩ + (⟨1, 1⟩ + (⟨1, 1⟩ +
...
lemma localLength (input : Var Inputs (F circomPrime)) (n : ℕ) :
    (main input).localLength n = 103012 := by
...
lemma circuit_localLength (input : Var Inputs (F circomPrime)) :
    circuit.localLength input = 103012 := by
...
theorem shape (input : Var Inputs (F circomPrime))
  ...
  · have hl : ∀ (i : Fin 64), Operations.localLength (stepBody input default i 0).2 = 1604 :=
...
lemma stepBody_localLength'' (input : Var Inputs (F circomPrime)) (acc : Var LazyPt (F circomPrime))
    (k : Fin 64) : (stepBody input acc k).localLength = 1604 :=
...
theorem structuralComputableWitnesses (o : ℕ) (input : Var Inputs (F circomPrime))
  ...
  all_goals
    ...
    have hax := hmap 8 (o + 63 * 1604 + 122 + 1450) (by omega)
    have hay := hmap 8 (o + 63 * 1604 + 122 + 1474) (by omega)
    simp only [circuit_norm, accL_succ, Step.outputAt, spE,
      MulCell.call_output, mulCell_output', mulCell_localLength',
      qbits, tbits,
